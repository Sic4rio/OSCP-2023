...
class BFSDumpShare(object):
    ...
    def dump_share(self, targetfile="", extensions=[], base_dir=None):
        if base_dir is not None:
            self.base_dir = base_dir
        print(f"[>] {TerminalColors.GREEN}Dumping files with extensions {extensions} ... {TerminalColors.RESET}")
        ...
        while len(searchdirs) != 0:
            ...
            for sdir in searchdirs:
                if self.debug:
                    print(f"[>] {TerminalColors.YELLOW}Searching in {sdir} {TerminalColors.RESET}")
                try:
                    for sharedfile in self.smb.listPath(self.share, sdir + "*", password=None):
                        if sharedfile.get_longname() not in [".", ".."]:
                            if sharedfile.is_directory():
                                if self.debug:
                                    print(f"[>] {TerminalColors.YELLOW}Found directory {sharedfile.get_longname()}/ {TerminalColors.RESET}")
                                ...
                                if len(extensions) == 0 or any([sharedfile.get_longname().endswith("." + e) for e in extensions]) or sharedfile.get_longname() == targetfile:
                                    if self.debug or not self.quiet or self.only_list_files:
                                        print(f"[>] {TerminalColors.GREEN}Found matching file {sdir + sharedfile.get_longname()} {TerminalColors.RESET}")
                                    ...
                                    if self.debug:
                                        print(f"[>] {TerminalColors.YELLOW}Found file {sharedfile.get_longname()} {TerminalColors.RESET}")
                except SessionError as e:
                    if self.debug:
                        print(f"[error] {TerminalColors.RED}{e} {TerminalColors.RESET}")
            searchdirs = next_dirs
            if self.debug:
                print(f"[>] {TerminalColors.YELLOW}Next iteration with {len(next_dirs)} folders.{TerminalColors.RESET}")
        return files
...
if __name__ == "__main__":
    ...
    try:
        ...
        if args.list_shares:
            ...
            print(f"{TerminalColors.GREEN}Shares on Remote Machine:{TerminalColors.RESET}")
            for s in shares:
                print(f"  - {TerminalColors.YELLOW}{s}{TerminalColors.RESET}")
            print()
        else:
            if args.dump_dir is None:
                args.dump_dir = f"./{address}/{args.share}/"
            ...
            if args.share in g.list_shares():
                if args.file is not None:
                    print(f"{TerminalColors.GREEN}Dumping file '{args.file}' from share '{args.share}'{TerminalColors.RESET}")
                    ...
                    if not args.list_files:
                        print(f"{TerminalColors.GREEN}Dumped {len(dumped_files)} files from share '{args.share}'{TerminalColors.RESET}")
            else:
                print(f"{TerminalColors.RED}Cannot find share '{args.share}'{TerminalColors.RESET}")
    except Exception as e:
        raise e
